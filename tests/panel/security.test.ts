...
describe("panel security (spec sections 3.1 and 3.2)", () => {
  it("requires --by even on the loopback interface", async () => {
    ...
    // append-only ledger.
    expect(() => parsePanelArgs([], { ...process.env,})).toThrowError(
      expect.objectContaining({ code: NO_VIEWER_IDENTITY }),
    );
    expect(() => parsePanelArgs(["--by", ""], { ...process.env,})).toThrowError(
      expect.objectContaining({ code: NO_VIEWER_IDENTITY }),
    );
  });
  ...
  it("defaults the bind address to the literal 127.0.0.1", () => {
    ...
    expect(parsePanelArgs(["--by", "amy"], { ...process.env,}).bind).toBe("127.0.0.1");
    expect(parsePanelArgs(["--by", "amy"], { ...process.env,}).confirmedExternal).toBe(false);
  });
  ...
  it("refuses a non-loopback bind by NAME when the exposure is not confirmed", () => {
    ...
    const opts = parsePanelArgs(["--by", "amy", "--bind", TEST_NET_1], { ...process.env,});
  ...
  it("lets a confirmed external bind through the guard", () => {
    const opts = parsePanelArgs(["--by", "amy", "--bind", TEST_NET_1, "--i-know-this-is-exposed"], { ...process.env,});
...
describe("parsePanelArgs argument validation (F6: task 3's deferred malformed-port / malformed-repo-argument)", () => {
  ...
  it("rejects --port by name for a non-integer, for -1, and for 65536; accepts 65535", () => {
    for (const bad of ["abc", "-1", "65536"]) {
      expect(() => parsePanelArgs(["--by", "amy", "--port", bad], { ...process.env,}), bad).toThrowError(
    ...
    expect(parsePanelArgs(["--by", "amy", "--port", "65535"], { ...process.env,}).port).toBe(65535);
  });

  it("rejects --repo by name for a value with no '=' and for one starting with '='; accepts k=path", () => {
    for (const bad of ["no-equals-here", "=path"]) {
      expect(() => parsePanelArgs(["--by", "amy", "--repo", bad], { ...process.env,}), bad).toThrowError(
    ...
    expect(parsePanelArgs(["--by", "amy", "--repo", "k=/some/path"], { ...process.env,}).repos).toEqual([
