...
async function fixture(mode = "ok", extra:Record<string,unknown> = {}) {
  ...
  const port=createCcloopExecutionPort({binary,adapter:"codex",adapterConfigPath:config,timeoutMs:2_000});
...
describe("production ccloop execution port",()=>{
  it("uses direct argv plus stdin JSON and validates successful responses",async()=>{
    ...
    const recorded=JSON.parse(await readFile(h.record,"utf8"));expect(recorded.argv).toEqual(["accept","--adapter","codex","--adapter-config",h.config]);expect(JSON.parse(recorded.stdin)).toEqual(h.envelope);
  });
