...
export interface ContinuationClaimInput extends CommandMeta {groupId:string;predecessorRunId:string;workItemId:string;taskId:string;graphVersion:number;targetVersion:number}
export function claimContinuation(store:ControlStore,input:ContinuationClaimInput):Claim {
 const {groupId,predecessorRunId,workItemId,taskId,graphVersion,targetVersion,...meta}=input;
 return applyCommand(store,groupId,meta,{verb:"continue",predecessorRunId,workItemId,taskId,graphVersion,targetVersion},()=>{
