...
export type ProjectionDependencies={writeProjection?:(path:string,bytes:Buffer)=>Promise<void>;admit?<T>(operation:()=>Promise<T>):Promise<T>};
...
async function drainPending(store:ControlStore,deps:ProjectionDependencies):Promise<void> {
 for(const row of store.db.prepare("SELECT id,kind,body FROM outbox WHERE kind IN ('projection','task-handoff','group-handoff') AND delivered=0 ORDER BY rowid").all()) {
  ...
  let publish:undefined|(()=>Promise<void>);
  if(row.kind==="projection") {const run=readRun(store,reference.runId);if(run.checkpointId===reference.checkpointId) {
   ...
   publish=async()=>{await (deps.writeProjection??writeProjection)(join(store.stateDir,"projections",run.runId,"latest.json"),Buffer.from(JSON.stringify({...c,hash:reference.hash})));};
   ...
   if(data.runId===reference.runId&&data.checkpointId===reference.checkpointId&&data.checkpointHash===reference.checkpointHash){const root=join(store.stateDir,"exports","groups",reference.groupId,"tasks",reference.taskId);publish=async()=>{await (deps.writeProjection??writeProjection)(join(root,"checkpoints",reference.checkpointId+".json"),built.json);await (deps.writeProjection??writeProjection)(join(root,"handoff.md"),built.markdown);};}
   ...
   if((reference.groupCheckpointId&&data.groupCheckpointId===reference.groupCheckpointId)||(!reference.groupCheckpointId&&data.revision===reference.revision&&data.budgetVersion===reference.budgetVersion)){const root=join(store.stateDir,"exports","groups",reference.groupId);publish=async()=>{await (deps.writeProjection??writeProjection)(join(root,"checkpoints",data.groupCheckpointId+".json"),built.json);await (deps.writeProjection??writeProjection)(join(root,"handoff.md"),built.markdown);};}
  }
  const commit=async()=>{await publish?.();store.transaction(()=>store.db.prepare("UPDATE outbox SET delivered=1 WHERE id=?").run(String(row.id)));};
