...
- *** **`.superpowers/sdd/**` 与已发布的 spec：历史一个字不改。** *** 写错了 ⇒ **另起一节记更正**，原文逐字保留。
  **这条是证据链的地基，不随下一条放宽。**
- ⚠️ *** **`docs/handoff/**` 是活文档：允许就地改、允许删、允许整节替换。** ***
  *** **它不该无限膨胀** *** —— 只追加不删，几轮之后没人读得完，**读不完的规则等于没有规则**。
  动别的 agent 落下的条目要**同时**满足三条：
  (a) 指得出**是什么让它过期的**（提交主题行／哪一次实测）；
  (b) 它承载的**结论**在留下来的段落里活着 —— **删的是过程，不是结论**；
  (c) 不把已知为假的说法带下去。**三条里有一条不确定就留着。**
