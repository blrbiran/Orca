...
> **它不是接手材料，每轮交接【不需要】读它** —— 定期对齐或方向有变时才拿出来过一遍。
> 本文只管「当前状态」与「下一件事」，两者不重复。
> *(加于 2026-09-22，会话 `da2f5e9a`，在主题行 `docs(handoff): roll the entry point onto the implemented assembly` 那一笔之上)*
...
**本轮实测数字**（命令、RC、红→绿全表在 `commands/2026-09-21-task-10.md`）：最终整支复验（带 `ORCA_CCLOOP_BIN` ＋ `ORCA_CCLOOP_ADAPTER_CONFIG`，在提交后的树上）`npm test` RC0＝**173 文件全通过、1519 测试全通过、零 skip**；`verify:web-control` 11 文件／112 passed／2 skipped；`:consumer` 4 passed（`ORCA_CCLOOP_BIN=/tmp/ccloop-codex-0919/dist/cli.js`）；`npm --workspace web run check` 11 文件／58 passed；`typecheck` 干净；`build --workspace web` `✓ built in 531ms`；`verify:control` 39 文件／403 tests；`verify:panel` `PASS 0`–`PASS 14`。三条判据更正都写进代码注释并归到生产事实上（`commands/` 的 RED 表），**判据文本一个字未自改**。
...
✅ *** **第 2 步已完成**（会话 `da2f5e9a`，2026-09-22）。 *** 两个姊妹仓各自**整节替换**了它们的
「Orca 那条线」（**不是追加**），提交主题行：
ccloop `docs(handoff): roll the Orca section onto ruling G1, and say what it does not settle`；
ccmem `docs(handoff): roll the Orca section, and drop two claims that outlived their facts`。
两节里都写死了 **G1 的边界**（只搬线上契约）与 **「G1 没裁 `targetVersion` 拍成什么」**。
⚠️ 两仓的提交**都没 push**，各自领先远端一笔。

**第 2.5 步 —— 压缩本文档**（人 2026-09-22 授权，Rule 13 已同步放宽）。
*** **本文现在 5300 多行，是三份 handoff 里最膨胀的一份。** ***
**方法定死如下，不许改成「按段落删」**：

1. **按【结论】重建，不是按【删段落】瘦身。** 通读十几轮的 `📌 本轮` 节，
   把每节里**还活着的结论**抽进一个常驻的「教训与既定事实」节。
2. **然后整段删掉过程日志** —— *** **每删一节，都要能指出它的结论落在新节的哪一条。** ***
   这让 Rule 13 的条件 (b)「结论要活着」变成**机械可查**，而不是凭感觉。
3. **三类东西的存活价值不同，别一视同仁**：当前状态（留）、
   一次性的踩坑教训（抽进常驻节）、纯过程日志（删）。
4. **粗估目标：一千行以内。** ⚠️ **这是目标，不是判据** —— 判据是第 2 条那句「指得出结论落在哪」。

⚠️ *** **压缩只动本文。** *** `.superpowers/sdd/**` 与已发布的 spec／注释**仍然一个字不改**。
