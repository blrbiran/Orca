...
### 九、本节写下之后又做的两件事（**同会话续写，上面一字未动；本节即为对第六节与第七节的具名更正**）

#### 1. 🔴 派了**第三席**，§14 又被整节重写了一次

第七节写的是「重写后的 §14 要不要再派一席 —— **人未裁**」。*** **人随后裁了：派，且修完【不再派席】，直接接 `writing-plans`。** ***

**第三席的 brief 是收窄的**（只两问）：这一轮的每个修法**又买来了什么**、判据表里**还有哪些不可能红**。
判 `Ready to implement? No`。**它两问都问出了东西**，报告归档在
`.superpowers/sdd/2026-09-06-corrections-store-and-writer/external-review-3.md`（带控制器复核）。

**最要紧的四条（四条承重主张控制器全部现测复核成立）**：

| | 内容 |
|---|---|
| **E6 没有任何变异能让它红** | 现测 `runId.ts` 哈希的是 `contractBytes ++ 0x00 ++ baseCommit`，而 §7.1 传的第三个入参是**含 `by` 的 correction id** ⇒ *** **`by` 经由两条路进派生** ***。上一版专门为「让变异有落点」做的改写，**落点被它上面一行的另一个入参抵消了** |
| **残留表与步骤表自相矛盾** | 上一版把写入挪到了闭-4，**却没回头改三段之后的残留表**（那里还写着「闭-1…闭-5 什么都没写」）⇒ `--decision` 打错字会**先写下一条永久垃圾行再拒**。**这条纯粹是文档一致性失败，不是设计错** |
| **部分提交的拒绝面更大** | *** **实测**：cherry-pick 冲突已解决未 `--continue` 时，`MERGE_HEAD` 不存在、无 rebase 目录、HEAD 未 detached、unmerged 为 0（**三条守卫全放行**），然后 `git commit -m … -- <path>` 得 `fatal: cannot do a partial commit during a cherry-pick.`，**EXIT=128** —— 而这发生在台账已经不可逆写完之后 *** |
| **两个新参数零判据覆盖** | 现测上一版 47 行判据表里 `undo-cost`／`undo-blast`／`blast_radius` **各 0 次命中** ⇒ 参数解析了但没接进派生对象，**31 条判据全绿** |

**处置**：21 条全收。§14 第三次整节重写（提交主题行
`docs(spec): rewrite section 14 again, after a third seat read the criteria table row by row`）。
**§0–§13 仍然逐字节未动**（`head -c 34731 | shasum -a 256` 四次都相同）。
台账 `.decisions/orca-dev-d5eb8fa6.jsonl` 现测 **18 条**，全部经 `appendEvent`。
`npm run verify` **exit 0**，**60 / 317**、**51 / 167**（全程未变）。

#### 2. 🔴 三条**新**记法（都是本轮实测出来的，第六节那五条仍然有效）

1. *** **「幂等」要先问清楚：它读的是【工作树】还是【仓库】？** *** 上一版的幂等检查看工作树文件 ⇒
   人在 W 上闭过环、切回主线重跑，**又写一遍**；而 C5 让两个 `at` 都是真实时刻 ⇒ 同一路径两个版本**逐字节不同**
   ⇒ 合并时**在那个「结构上不可能冲突」的只追加台账文件上产生内容冲突**。
   现已改成分支无关的两问（工作树 ＋ `git log --all`）。
2. *** **一条被登记为【预期】的失败，必须有自己的退出码。** *** 上一版为「提交被钩子拒」建了恢复路径却没给码，
   而四个现有码没有一个说得对（`1` 说重试没用、`4` 说有锁、`3` 说未预期，而登记项自己说它是预期的）。现已新增 **exit 5**。
3. *** **一个新参数如果没有判据，等于没加。** *** 一行的疏漏就能让整张判据表全绿而参数从未生效。
   ⇒ **每新增一个用户可见的输入，同一轮里就给它一条判据。**

⚠️ 🔴 **外加一条本节自己踩出来的**：*** **写下一条记法，和执行一条记法，是两件事。** ***
§14.23 第 7 条刚写完「改了一张表就要扫所有引用它的地方」，**同一次收尾自查就发现 §14.0 指着一个当时并不存在的 §14.24**。
⇒ 现在的做法是**机械扫**：把文中每一个 `14.x` 引用拿去比对实际小节标题（现测：25 个小节，**零悬空引用**）。

#### 3. ⛔ 下一件事（**第七节那张表的第 1、2 条就此更新**）

| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | *** **接 `writing-plans`** *** | **人已裁：不再派席。** 理由：真正还能检验这份设计的不是第四席，是 **§10 那条验收 —— 在一次性仓库里让第一条真 `overturned` 落盘并被 `orca validate` 判 ok** |
| **2** | `CLAUDE.md` 加「用户全局数据」条款 | 人已裁：**spec 定稿后单独一笔**。§14 现已定稿 ⇒ **这一条可以做了** |
| **3–7** | 与第七节那张表的第 4–7 条相同（裁决甲的 `plan` 那一半、子系统 D、ccloop 的 E1、未 push） | 未变 |

#### 4. 成本（**对第八节的补充**）

**第三席**：*** **218,207 token ／ 38 次工具调用 ／ 855,587 ms**（派出方的工具报数）。 ***
⚠️ **它自己说 harness 不给它计数、拒绝自估** —— **派出方这一侧拿得到，就从那里抄。**
⇒ **三席累计 133,275 ＋ 164,453 ＋ 218,207 token。** *** **每一席都比上一席贵，而且每一席都找出了真缺陷。** ***
**会话总花费**：钩子最后一次报出的是**约 $21.54**（本轮中段），此后没再报过 ⇒ **不写，不自估。**

### 十、收尾（**同会话续写，上面一字未动；本节是对第九节第 3 项的具名更正**）

第九节第 3 项把「`CLAUDE.md` 加用户全局数据条款」列为下一件事的第 2 条。*** **它已经做完了。** ***

| 做完的 | 内容 |
|---|---|
| **`CLAUDE.md` 新增 Rule 17**（提交主题行 `docs: add the rule for writing outside the repository, now that a command will`） | 「写到仓库外面去的东西，判据不许碰真的那一份」。承重的是这句：*** **一条会写进使用者真实用户数据的判据，是不可接受的，哪怕它「只写一行」。** *** 另两条：新建目录／文件**显式给 mode**（`0700`／`0600`），不从 umask 继承、**不改已存在文件的 mode**；仓库外的写入方要在 spec 里**具名登记**。**现测 143 / 200 行，机器检查通过** |
| **ccloop 的 Orca 章节已整节重写**（提交主题行 `docs(handoff): rewrite the Orca section in place -- orca correct's shape is settled, and one git fact worth keeping`） | 不新增编号章节；**节外内容 sha256 前后相同**。新增两条：`orca correct` 的形状已定（闭环模式取目标仓库 repo 锁、只提交它写的那一个文件、只记模式不取锁）；以及那条 git 实测，**并替它现测过本仓库没有这个形状**（唯一的 `"commit"` 调用点是 `worktreeManager.ts` 的 `add -A` ＋ 一次性 attempt worktree，有意为之） |
| **ccmem 本轮零触碰** | 人明确说**另一个 agent 正在改 ccmem**，本轮不写它 |

**⛔ 下一件事因此收窄成一条**：*** **接 `writing-plans`。** *** 其余（裁决甲的 `plan` 那一半、子系统 D、
ccloop 的 E1 ＋ 人裁 85、未 push 的提交）与第九节第 3 项那张表相同。
...
### 七.1 一处具名更正（**同会话追加；上面一字未动**）

上面第 6.1 节第 3 项写着 `.superpowers/sdd/2026-09-07-corrections-store-and-writer/progress.md`
**「没有进 git；它随工作区存在，不随仓库」**。*** **写下时为真，随后被本会话自己改掉了。** ***

**改法与理由**：本节第三条把 spec §14.3 的守卫表判为「已知部分为假」，而那条判决的证据
（Step 0 的完整实测表）当时只活在工作区里 —— *** **一份下一位可能根本拿不到的证据，不足以支撑一条推翻 spec 的结论。** ***
按本仓库历轮惯例（`.superpowers/sdd/` 的 `.gitignore` 是 `*`，材料用 `git add -f` 归档），已把两份归档进仓库：

- `…/progress.md` —— **本轮全部裁决的流水**，每条带「如果判错了代价是什么」；
- `…/task-9-report.md` —— **守卫表的完整实测**，即第三条那条更正的第一手依据。

⚠️ *** **`git add -f` 必须单独成一条命令。** *** `git add <已跟踪文件> <被 gitignore 的文件>` 会把能加的加进去
**同时以非 0 退出**，于是 `git add … && git commit …` 这条链里暂存已经发生、提交却被 `&&` 跳过 ——
这个坑上一轮真的踩过（记在 2026-09-06 那一节第五节）。本轮两条 `git add -f` 是分开跑的。

⚠️ 其余报告（各任务的实施报告、全分支评审与修复波的报告）**仍然只在工作区**，未归档 —— 它们的结论
已被本节与台账吸收，不构成任何推翻性判决的唯一证据。

---

# 📌 本轮（2026-09-08／09，会话 `ad1e30c6`）—— **E2 与 E3 的 spec 都定稿了；两席外审各判一次 `No`，两次都把我写的话现测成假的**

**归属**：run `orca-dev-ad1e30c6`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD、不写「领先几笔」** —— 提交本文就会改这两个数，人也会自己推远端。
**要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`。**

## 一句话状态

*** **上一轮的「下一件事 #2」（给 spec 追 ERRATUM）做完了；随后 brainstorm 出【两份】新 spec：E2（指标核心）与 E3（面板），各经一轮自审 ＋ 一席外派评审，各被整份重写一次。** ***
**产品代码本轮只动了一处**（`storeLock.ts` 的挂钟 deadline，带变异证明）。
**一次都没 push、没建分支、没合并、没删任何分支或 worktree。**

## 一、开工与收尾的现测（都带命令）

| 项 | 开工（锚点：`1b73923`） | 收尾（锚点：`a39815a`） |
|---|---|---|
| `rtk proxy npm run verify` | **exit 0**，全仓 **73 files / 397 tests**、scheduler **51 / 167** | **exit 0**，全仓 *** **73 / 398** ***、scheduler **51 / 167** |
| `git status --porcelain` | 0 字节 | 0 字节 |
| `ls ~/.orca` | 不存在 | *** **仍然不存在** *** |
| `git ls-remote origin refs/heads/main` | `3f9a1ae`（是本地 HEAD 的祖先） | **同上，人本轮没推过** |

⚠️ *** **`verify` 打印两个判据数，别混着比**（73/398 是全仓，51/167 是 scheduler 那一档）。 ***
**+1 条就是本轮新增的那条时钟判据；scheduler 档全程未变，因为没有任何改动落在 `tests/scheduler/`。**

## 二、做出来的东西（**按提交主题行找，别数笔数**）

| 买到的东西 | 提交主题行 |
|---|---|
| 上一轮欠的 ERRATUM（spec §14.3 守卫表）。**§0–§14 用 `cmp` 证明逐字节未动** | `docs(spec): append the erratum that says the guard table was measured, not designed` |
| 已发布注释的具名 ERRATUM | `docs(corrections): erratum the comment that says nothing has been built yet` |
| 🔴 **挂钟量时长 → 注入单调钟**（唯一的生产代码改动） | `fix(storeLock): stop measuring an elapsed second with a clock that can be set` |
| **E2 spec**（指标核心，只读） | `docs(spec): design the metrics core…` → `…fix what an adversarial self-review found…` → `…rewrite E2 after a review seat took the decomposition apart` |
| **E3 spec**（面板） | `docs(spec): design the panel…` → `…fix six self-review findings…` → `…rewrite the panel after a seat measured three of its claims false` |
| 🔴 **两条高位框架决策落进台账** | `feat(decisions): record the two framework choices the spec had left as parentheses` |

台账 `.decisions/orca-dev-ad1e30c6.jsonl` **现测 2 条**，全部经 `appendEvents`，校验器零抱怨。

## 三、🔴 六条**必须带给下一轮**的实测（别重新发现）

### 1. *** 「照抄 X」是一条【可现测】的断言，不是背景说明 —— 本轮犯了两次 ***

- **第一次**：E3 spec 写「前端住独立工作区，`npm ci` 不会装 `web/` 的依赖」，并在它上面建了一整条代价链（要装两次、一条登记项、一条变异、第二条 symlink）。
  **外审现测**：`hermes-agent` 根 `workspaces` 含 `"web"`、**根有 `package-lock.json`**、**`web/` 没有自己的 lockfile**；`openclaw` 的 `pnpm-workspace.yaml` 含 `ui` 且 `ui/` 无自己的 lockfile。⇒ *** **那个结构是我发明的，代价链整条作废。** ***
- **第二次**：同一份 spec 写「后端 `node:http`，不引 express」并暗示照抄。**人追问后现测**：`openclaw` 根 `package.json` 有 `express: 5.2.1`，`hermes-agent` 用 **FastAPI ＋ uvicorn**。⇒ **两个参考都是反着做的。**
- ⇒ 🔴 **推论（比记法本身更值钱）**：*** **「X 做了什么」和「X 的前提我们有没有」是两件事，两件都要现测。** ***
  后端这一格两个参考都用框架，但 `openclaw` 根依赖 **115** 个，本仓库是 **1** 个 —— 边际成本差一个数量级。**人看到这两条之后仍裁定引 express，代价具名登记。**

### 2. *** 一条判据可以在真实世界里造成它所测试的那个危害 ***

E3 的绑定守卫判据第一版写成 `--bind 0.0.0.0` ⇒ **变异跑（守卫被删掉）时，它会真的在所有网卡上起一个能读写使用者全局 corrections 的服务。**
⚠️ **本仓库历轮的纪律都在防「判据【碰】真实用户数据」（Rule 17），这是新的一种：判据【制造】真实攻击面。**
⇒ **新记法：写守卫判据时问一句 —— 这条判据在【守卫被删掉】的那次跑里，会做出什么？**
⇒ 改法：用 `192.0.2.1`（RFC 5737 TEST-NET-1）。⚠️ **但光换地址不够** —— 换了之后有守卫是「具名拒绝」、没守卫是 `EADDRNOTAVAIL`，**两者都没在监听** ⇒ **必须同时断言是【哪一种】拒绝**。
⚠️ 外审另现测：`192.0.2.1` 上 **connect 拿不到 `ECONNREFUSED`**（它沿默认路由发 SYN 然后超时）⇒ **「没在监听」只能用操作系统侧观测，不能用 connect。**

### 3. *** 同一轮的两个修法会互相拆台 ***

E3 的 I1 修法让 `--by` **永远必填**；C2 修法那条判据却写着「**不给 identity**」
⇒ **命令在参数解析处就退出，根本走不到 bind 守卫；删掉 bind 守卫，判据照绿。**
⇒ **修完一批之后，把每条修法拿去和同一批里的其它每一条对撞一次。**

### 4. *** 把观测点挪到服务端，防的是【伪造】，不是【噪声】 ***

E3 第一版论证「服务端判定 `expanded` 排除了误触与批量展开」。**排除不了** —— React 版「选中行即取详情」，方向键下按 20 行 ＝ 20 条记录。
⇒ *** **改变的是【谁报告】这个事件，不是【什么造成】它。而覆盖率这类指标的威胁从来是噪声。** ***
⇒ 改法：`opened`（噪声，不承重）／`reviewed`（刻意动作：记 correction 或点「同意」，**承重**）。**这个定义与框架无关。**

### 5. *** 「绿」的第六种坏法：判据对、变异也对，但两者之间隔着一条【时间上跑得比它慢】的断言 ***

`storeLock` 那条时钟判据：变异（回退成 `Date.now()`）下 *** **「它拒绝了」那条断言照绿** *** ——
变异体真的跑满 1000ms 才超时（实测 1006ms，对比绿时整个文件 199ms）。
红的是**读数表 `[]`**（注入的时钟一次都没被问过）。
⇒ **凡是涉及时钟的判据，只断言「跑出来了／拒绝了」一定是空的 —— 要断言【哪个时钟被问了】或【年龄的字面值】。**

### 6. *** 一份 spec 里「今天不需要 X」和「X 会拿走什么」不能同时存在 ***

E2 第一版 §10 写「今天扫描买得到 DB 买得到的一切」，同一份文档的 §8 又替 DB 排了期。**外审第一眼就看见。**
⇒ **凡是写下「今天不需要」，就去删掉所有替它排期的句子。**
⇒ 落地结果：**E4（DB）不出 spec，改为由一条实测阈值触发** —— 现测全量扫 `.decisions/` **144 行 ＝ 2.4 ms**（外审计时，控制器复核了行数与体积 192K）。

## 四、两份 spec 的形状（**读之前先读这两句**）

| | 路径 | 一句话 |
|---|---|---|
| **E2** | `docs/superpowers/specs/2026-09-08-metrics-core-and-query-design.md` | *** **纯只读**：不写任何文件、不取任何锁。写入方从第一版的 2 个降到 **0** *** |
| **E3** | `docs/superpowers/specs/2026-09-09-panel-design.md` | React（`web/` 进根 `workspaces`）＋ **express** 后端；**面板只能记，不能闭环**（A′ §4.1） |

⚠️ **两份都各有一节【外审记录】（E2 §12、E3 §10），里面逐条记着「席位说了什么／控制器复核结果／怎么处置」。**
*** **其中 E2 §12.1 的 C6 那一格记着一次「席位的谓词站不住，但结论成立，且复核后的数字比它自己报的更硬」** *** —— 下一轮读评审报告时照这个姿势读。

⚠️ **E3 §8 给自己派了一件开工前置**：框架选型必须先进台账。**已做**（本轮台账两条）。
⚠️ **E3 §9 第 6 项**：提 correction seam 是**改既有生产代码**，*** **开工前需人另拿一次具名授权。** ***

## 五、本轮**没有**做的（登记，不掩饰）

- **未 push、未建分支、未合并、未删任何分支或 worktree。**
- **ccmem 零触碰**；**ccloop 本轮零触碰**（连它的 Orca 章节都没更新 —— 见下）。
- **E4（DB）不出 spec** —— 有意的，见第三条第 6 项。
- **两份 spec 都还没有实施计划**（`writing-plans` 没跑）。
- **裁决甲的 `plan` 那一半没做**（仍需人指名授权改 `preflightUnreadableRepo.test.ts`）。
- **子系统 D 没开**；**B 的后续没动**。
- **ccloop 的 E1 的 I-2 ＋ 人裁 85 没动**（人裁 121 仍有效）。

## 六、⛔ 下一件事

| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | *** **未 push 的提交等人单独授权** *** | **控制器不许 push。** 人本轮明确说过「先不 push」。哪些已发布**必须现跑** `ls-remote` ＋ `merge-base --is-ancestor` 自己判 |
| **2** | **接 `writing-plans`，先做 E2** | 两份 spec 都定稿。⚠️ **E2 先做**：E3 §9 第 8 项写着它建在未实施的 E2 上，若实施推翻则**给 E3 追具名 ERRATUM，不就地改** |
| **3** | 更新 ccloop 的 Orca 章节 | **本轮没做。** 那边的规矩是**整节重写、不新增编号章节**。值得带过去的是本节第三条第 1、2、5 项 —— **三条都与语言无关，ccloop 也在做变异** |
| **4** | 子系统 **D**，或 B 的后续 | 未变 |
| **5** | 裁决甲的 `plan` 那一半 | 仍需人**指名授权** |
| **6** | ccloop 的 **E1 的 I-2 ＋ 人裁 85** | 人裁 121 仍有效 |
...
1. **本节第三条的六条实测**（尤其第 1 项那条推论、第 2 项那种新的判据坏法）。
2. **两份 spec 的外审记录节**（E2 §12、E3 §10）—— 哪几条是现测复核成立的、哪一条席位的证据烂但结论对。
3. `.decisions/orca-dev-ad1e30c6.jsonl` **2 条**，都是高位 `dependency` 决策，`why_not` 里带着外审的反对意见原文。
4. **E3 §9** 的八条登记项 —— 其中第 6 项是开工前置（需另拿授权）。

## 七、成本

**只抄钩子报出来的数**（Rule 14）：钩子在本轮报过三次 —— **约 $43.74**（派第一席之前）→ **约 $131.04**（第一席回来 ＋ E2 重写 ＋ E3 全程）→ *** **约 $178.86** ***（第二席回来之后）。
**此后没有再报过，因此不写 —— 不许自估。**

**两席外派评审的用量（派出方工具报数）**：
- 第一席（E2）：**139,447 token ／ 29 次工具调用 ／ 795,843 ms**
- 第二席（E3）：**155,541 token ／ 20 次工具调用 ／ 811,814 ms**

⇒ *** **一席外派的量级仍稳定在 130k–220k token，与上一轮三席一致。** ***
⚠️ *** **两席都判 `Ready to implement? No`，且两席找出的最贵那几条，全是「把 spec 自己写的话现测成假的」** *** ——
**不是设计品味问题，是事实核对问题。派席买到的就是这个。**

## 八、🔴 对本节第一表与第五节的**具名更正**（**同会话收尾时追加；上文一字未动**）

**本节第一表写着**：`git ls-remote origin refs/heads/main` 收尾栏 ＝「`3f9a1ae`，**同上，人本轮没推过**」。
*** **那句是假的。** *** 收尾现测远端是 **`e1a356a`**。

**现测依据**（`.git/logs/refs/remotes/origin/main`，`update by push`，人自己推的）：
*** **人在 2026-09-09 08:20:08 推过一次，把远端从 `3f9a1ae` 带到了 `e1a356a`。** ***

⇒ **随之被更正的**：
1. 本节各处报的「未 push N 笔」**都是拿开工那个陈旧基点算的**。
   ⚠️ *** **本节【不写】未 push 的笔数** *** —— 提交本文这个动作本身就会改它，人也会自己推远端。
   **要判就现跑**：`R=$(git ls-remote origin refs/heads/main | cut -f1); git rev-list --count $R..HEAD`。
   收尾观测时远端**是**本地 HEAD 的祖先，可 `--ff-only`；**这一句同样只在观测的那一秒为真。**
2. 第五节「未 push」那一格因此**读法要改**：本轮绝大多数提交**已经发布了**，只是**不是控制器推的**。

### 8.1 🔴 它连带暴露了一次**真实的铁律违反**（登记，不找补）

**面板 spec 在 `e1a356a` 那一刻就发布了，而我随后【就地改了两次】**（`7b60ad3` 08:26:33、`c350c37` 08:49:18），
**两次都在文内断言「本文当时未发布」，而两次都没有重新现测。**
⇒ 违反注释铁律（**已发布 ⇒ 原文逐字保留 ＋ 追加具名 ERRATUM，不许就地改**）。

**补救已做**：`docs/superpowers/specs/2026-09-09-panel-design.md` **新增 §12**，
把被推翻的六句**逐字取自 `e1a356a`** 补回文件里，并记下经过。**只追加，52 增 0 删。**
⚠️ **E2 那份不受影响** —— 现测它的三笔就地改全部在 08:20 之前。

### 8.2 *** 这是本轮最该被下一位继承的一条 ***

*** **「本文未发布」是一条【会过期的现测】，不是文档的属性。** ***
本轮那次推送，**恰好落在两次重写之间的 2 分 29 秒里**。

⇒ **机械做法，不许省**：
1. *** **每一次「就地改」之前，现跑一次 `git ls-remote` ＋ `merge-base --is-ancestor`。开工那次的结果不能复用。** ***
2. **写下「未发布」时，把那次现测的【远端 sha】一起写上** —— 没有 sha 的「未发布」是不可复核的断言。
3. ⚠️ *** **收尾的 `ls-remote` 不只是为了报「还剩几笔没推」，它同时是「我这一轮有没有改过已发布文本」的唯一检测手段。本轮就是靠它才发现的。** ***

---

# 📌 本轮（2026-09-09，会话 `3738db1c`）—— **E2 已实施完毕**；两席外审各判一次 `No`，第二席审的是第一席那些【修法本身】

**归属**：run `orca-dev-3738db1c`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD、不写「领先几笔」** —— 提交本文就会改这两个数，人也会自己推远端。
**要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`。**

## 一句话状态

*** **`orca metrics` 存在了 —— 第六个子命令，纯只读，一个字节不写、一把锁不取。** ***
路径是：还清 A′ 欠的三条 ERRATUM → 出 E2 计划（v1）→ 一席外审判 `No`（7C/9I/6M）→ 出 v2 →
第二席审**修法本身**判 `No`（2C/6I/8M，但 22 条修法里 **16 条现测修好**）→ 修完 → 实施。
**未 push、未建分支、未合并、未删任何分支或 worktree。**
...
| 买到的东西 | 提交主题行 |
|---|---|
| A′ 欠的三条 ERRATUM（3／4／5）一次还清，**101 增 0 删** | `docs(spec): settle the three errata A' was owed, without touching a published line` |
| E2 计划 v1 ＋ 三条替 spec 做的裁断进台账 | `docs(plan): turn E2 into eight tasks, and record the three calls the spec left open` |
| 第一席的判决与 ⛔ 阻断横幅（**120 增 0 删**） | `docs(plan): record the seat's verdict on this plan without touching a line of it` |
| **计划 v2**（自包含，2000+ 行） | `docs(plan): rewrite E2 as v2, with the shell that would have deleted the worktree gone` |
| 第二席的 8 条实质发现全部修完 | `docs(plan): give every mutation in v2 a place it can actually go red` |
| 🔴 **E2 实施**（21 个文件，0 删除） | `feat(metrics): add orca metrics, a sixth subcommand that reads everything and writes nothing` |

**材料**：计划 `docs/superpowers/plans/2026-09-09-metrics-core-and-query-v2.md`（**v1 已作废，带 ⛔ 横幅与 ERRATUM 1**）；
两席报告 ＋ 控制器复核表 `.superpowers/sdd/2026-09-09-metrics-core-e2/external-review{,-2}.md`
（⚠️ 该目录 `.gitignore` 是 `*`，是 `git add -f` 进来的）。
**台账 `.decisions/orca-dev-3738db1c.jsonl` 现测 6 条**，`orca validate` 单独跑 exit 0。

## 二、实测数（**只抄工具打印的**）

| 项 | 开工 | 收尾 |
|---|---|---|
| `rtk proxy npm run verify` | exit 0，全仓 **73 files / 398 tests** | exit 0，全仓 *** **81 / 459** *** |
| 同上 scheduler 档 | **51 / 167** | **51 / 167**（未变 —— 没有任何改动落在 `tests/scheduler/`） |
| `git status --porcelain` | 0 字节 | 0 字节 |
| `ls ~/.orca` | 不存在 | *** **仍然不存在** ***（Rule 17 的活证据） |

⚠️ *** **两个判据数别混着比** *** —— 81/459 是全仓，51/167 是 scheduler。**第一席就在这栽过。**

## 三、🔴 六条**必须带给下一轮**的实测（别重新发现）

### 1. *** 计划里写下「应当红／应当绿」，就是写下一条【预言】—— 而我的两条预言现测都是假的 ***

v2 第一版写着「变异 10 跑完确认另一条**仍然绿**」（实际必红，因为 `asked` 变成 `[]`），
以及「变异 6 期望红在 `stdout.length` 那条」（实际到不了 —— `expect(result).toBe(6)` 排在前面会先炸）。
⇒ *** **两条都会【主动把执行者引向错误结论】** *** ——
按纪律「红在别处＝假红」，执行者会回去改没坏的代码。
⇒ **记法：每写一条「期望红在 X」，就把那条变异在脑内跑到底，问「在 X 之前有没有别的断言会先炸」。**
⚠️ 这是上一轮「Self-Review 会说可现测的谎」的**下一层**：v2 把「核过」换成了可跑的 `tsc`，
**却没有把「会红」也换成可跑的验证。**

### 2. *** 一条变异要能红，先问「被删掉的那件事，是不是【唯一】在做它的那件事」 ***

实施时实测：变异 M9 落上去了（sha 变了）**却照绿**。顺着查发现守卫冗余，
再顺着查发现被它遮住的是**一个真的错误归桶** —— 纠正率按 kind 归桶时用 `decisionId` 全局查，
而 `decision id` 是 `<run-id>/<n>`、**不含任何仓库身份**（`corrections/schema.ts` 的注释正是用这条论证
correction 必须自带 `projectKey`）⇒ 同一个 id 在 clone 与 fork 之间真的会重复。
**改成 `projectKey ＋ decisionId` 联合键，删掉冗余守卫，判据侧配一条第三仓库同 id 的夹具。**
（台账 `orca-dev-3738db1c/6`。）

### 3. *** 计划正文里的 shell 是自审看不见的那一块 ***

第一席抓到的最危险一条**不在设计层**：v1 四行 `/bin/rm -rf "$(dirname "$D")"`，
而 `D=$(mktemp -d)` 每次都在**另一个代码块**里 —— *** **shell 状态不跨 Bash 调用持久** *** ⇒
四处全部展开成 `/bin/rm -rf .`。`writing-plans` 的自查三项（spec 覆盖／占位扫描／类型一致）**结构上都不看 shell**。
⇒ **Shell scan 要【两问】，且都写成可跑的命令**：
1. **变量未赋值时它展开成什么？**（护栏是 `"${VAR:?msg}"`）
2. *** **有没有 `<` 或 `>` 出现在不是重定向的位置？** *** ——
   现测：占位符文件**存在**时 `… --root <夹具> --as-of X > out.json` 会**造出一个名叫 `--as-of` 的文件**；
   不存在时 bash 直接失败。**第一问结构上看不见第二问（那里根本没有变量）。**

### 4. *** 扫描器不许对自己的警告文字报警 ***

本轮踩了**两次同一个坑**：计划里的 shell 扫描器命中了它自己那句「别用 `<…>`」的注释；
`compute.ts` 的纯度判据命中了它自己那句「there is no `new Date()`」的注释。
⇒ **两处都改成「先剥注释再判」。** *** **一个对自己注释报警的扫描器，会被读的人学会忽略。** ***

### 5. *** 夹具的 schema 违规不会响，它会伪装成坏行 ***

§7 夹具那条 `not_my_taste` 忘了带 `chose_instead`（`correctionSchema` 的 superRefine 只对它强制）。
**没有任何东西报错** —— 宽容读取器把它判成坏行，那条 correction 悄悄退出全部计数，
**还把一个临时目录路径写进了即将定稿的 golden**（那样 golden 永远不可复现）。
⇒ **生成 golden 之后必须整份读一遍，并逐个数对着夹具的已知条数核。**

### 6. **两席都判 `No`，而两席买到的东西形状不同**

第一席买的是「把计划自己写的话现测成假的」（22 条）；**第二席买的是「修法之间的对撞」**——
它按要求做了两两对撞（撞了 14 组，撞出 3 组），其中一组是
*** **两个各自正确的修法合起来让一条变异变成空操作** ***（集合序由 compute 的 `sort` 守、
字段序两处同序 ⇒ 去掉渲染层白名单**一条判据都不红**）。
⇒ **派第二席时就该指名要它做对撞** —— 这是它独有的产出。

## 四、E2 长什么样（**读代码之前先读这三句**）

| 层 | 文件 | 一句话 |
|---|---|---|
| 纯 | `src/metrics/resolve.ts` | correction ↔ decision ↔ overturned 对号；**归档查找的 fs 是注入的** ⇒ 判据能**数它被问过几个路径** |
| fs | `src/metrics/collect.ts` | 仓库发现、读文件、`--as-of` 过滤、把 fs 接到纯层上。**`fsArchiveIo` 是 export 的**，因为「一层查偷偷变全扫」的风险在这一侧 |
| 纯 | `src/metrics/compute.ts` | 口径的唯一持有者，**零 fs／零时钟／零 git** |
| — | `src/metrics/report.ts` | `renderJson` 按 `METRICS_FIELDS` 白名单逐字段取值；**耗时只走 stderr** |
| — | `src/cli.ts` | `orca metrics`，**exit 6 ⟺ 存在至少一条 `torn === false` 的坏行** |

⚠️ *** **`METRICS_FIELDS` 的顺序【故意】与 `MetricsReport` 的声明序不同** *** ——
否则「去掉白名单」这条变异产出的 key 序不变、判据照绿（第二席 C-2）。**别顺手把它排整齐。**
⚠️ **`review_coverage.available` 是有意的常量**（生产者在面板那一刀）；承重的是它逼进两个率的那句 caveat，**不是这个布尔**。

## 五、本轮**没有**做的（登记，不掩饰）

- **未 push、未建分支、未合并、未删任何分支或 worktree。**
- **ccmem 零触碰**（除本轮末尾对它 §15 的滚动更新）；**ccloop 只更新它的 Orca 那一节**。
- **21 条变异跑了 13 条**（其中 M9 拆成两种形态各跑一次）。**没跑的 8 条**：
  5a/5b（`--as-of` 两模式）、7a/7b（两处 push）、8、11、12、13a/13b、15（两步 tsc）、18。
  *** **它们各自的判据是绿的，但「判据承重」这一格没有被看见过。** *** **登记，不假装跑过。**
- **E3（面板）未开**；**E4（DB）仍由实测阈值触发，不出 spec**。
- **裁决甲的 `plan` 那一半没做**（仍需人指名授权改 `preflightUnreadableRepo.test.ts`）。
- **ccloop 的 E1 的 I-2 ＋ 人裁 85 没动**（人裁 121 仍有效）。
...
| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | *** **未 push 的提交等人单独授权** *** | **控制器不许 push。** 哪些已发布**必须现跑** `ls-remote` ＋ `merge-base --is-ancestor` 自己判 |
| **2** | **把剩下 8 条变异补跑完** | 计划 v2 每条都写了锚点与期望；模板在「变异模板」一节。⚠️ **跑之前先读第三条实测（shell 两问）** |
| **3** | **E3（面板）** | ⚠️ **开工前置**：E3 §9 第 6 项的 correction seam 是**改既有生产代码**，*** 需人另拿一次具名授权 ***（本轮人未授权，我明确没有把「同意，继续」当成它） |
| **4** | 子系统 **D**，或 B 的后续 | 未变 |
| **5** | 裁决甲的 `plan` 那一半 | 仍需人**指名授权** |
| **6** | ccloop 的 **E1 的 I-2 ＋ 人裁 85** | 人裁 121 仍有效 |
...
1. **本节第三条那六条实测**，尤其第 1、2 条 —— 它们是本轮花两席钱买到的。
2. **计划 v2 开头那张 22 行表** ＋ 它末尾的「第二席外派评审」一节：哪些修法是怎么被撞出来的。
3. **两份外审报告的「控制器复核」节** —— 哪几条现测成立、哪一条席位的证据烂但结论对。
4. `.decisions/orca-dev-3738db1c.jsonl` **6 条**：`/1`–`/3` 是替 spec 做的裁断，
   *** **`/4`、`/5` 是对 `/1`、`/2` 的更正**（依据被外审现测成假）***，`/6` 是实施时跑变异跑出来的。
   ⚠️ **没有写 `overturned`** —— 它需要一个 correctionId，而 correction 是**人**的日志，
   *** **agent 不许替人编一条** ***（那等于伪造喂给 ccmem 的对照样本的右半边）。

## 七、成本

**只抄钩子报出来的数**（Rule 14）：钩子在本轮报过 **约 $10.70** → **约 $51.51** → **约 $81.06** → **约 $104.99** → *** **约 $133.04** ***。
**此后没有再报过，因此不写 —— 不许自估。**

**两席外派评审的用量（派出方工具报数）**：
- 第一席（审 v1）：**185,683 token ／ 33 次工具调用 ／ 763,287 ms**
- 第二席（审修法）：**188,123 token ／ 27 次工具调用 ／ 906,568 ms**

⇒ *** **一席稳定在 130k–220k token，连续四轮成立。** ***
⚠️ **两席都拿不到自己的 token 数**（自陈「拿不到」）—— **那一格只能由派出方的工具报数填**。

---

# 📌 本轮（2026-09-10，会话 `30fa920f`）—— **上一轮欠的 8 条变异全部补跑完**；三条「期望红在 X」写得不全

**归属**：run `orca-dev-30fa920f`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD、不写「领先几笔」** —— 提交本文就会改这两个数，人也会自己推远端。
**要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`。**

## 一句话状态

*** **E2 的 21 条变异现在【条条被看见红过】了。** *** 上一轮登记「没跑的 8 条」（5a/5b、7a/7b、8、11、12、13a/13b、15、18）
本轮全部补跑：**11 次单跑，全部见红，无一跑绿** ⇒ **没有发现新的真缺陷**（不同于上一轮的 M9）。
**生产代码一行未改，判据一行未改**，只多了一节 ERRATUM。**未 push、未建分支、未合并、未删任何分支或 worktree。**

## 一、🔴 开工核对推翻了交接给我的第一件事

| 项 | 命令 | 结果 |
|---|---|---|
| 远端 vs 本地 | `git ls-remote origin refs/heads/main` ／ `git rev-parse HEAD` | *** **同点** *** |
| 工作树 | `rtk proxy git status --porcelain > f; wc -c < f` | **0 字节** |
| 全量判据 | `rtk proxy npm run verify > f 2>&1` | `VERIFY_RC=0`，全仓 **81/459**，scheduler **51/167** |
| Rule 17 活证据 | `ls ~/.orca` | **不存在** |

*** **上一轮「⛔ 下一件事」第 1 条（未 push 的提交等人单独授权）已经不成立** *** ——
人自己把那 7 笔（`docs(spec): settle the three errata A' was owed…` 起到 `docs(handoff): record the round that shipped orca metrics…` 止）推上去了。
⇒ 本轮开工时**整个 E2 已是已发布文本**，所以本轮对计划 v2 的更正**只能追加 ERRATUM 2，不许就地改**。
⚠️ **这条又一次兑现了 ccloop 那条记法**：「本文未发布」是**会过期的现测**，不是文档属性。

## 二、做出来的东西（**按提交主题行找，别数笔数**）

| 买到的东西 | 提交主题行 |
|---|---|
| 8 组变异补跑 ＋ 三条不全预言的更正 ＋ Q1 扫描器的修正版（**137 增 0 删**） | `docs(plan): run the eight mutations that were only ever assumed to be load-bearing` |

**材料**：计划 `docs/superpowers/plans/2026-09-09-metrics-core-and-query-v2.md` 的 **ERRATUM 2**（第 2209 行起）。
*** **上面 2208 行逐字节未动** *** —— `sha256(第 1..2208 行)` 在本轮前后相同（`9d0022a5…`），已现测。

## 三、11 次变异单跑的实测（**只抄工具打印的**）

每条都做了四件事：`shasum` 前后不等 ＋ 见红 ＋ **读一眼红在不在点名那条** ＋ `/bin/rm -rf "${T:?…}"` 销毁副本。

| 变异 | 落点 | 红在 | 与计划的期望 |
|---|---|---|---|
| 15 步一 | `highTier.ts` 松 `Record` 约束 | `TSC_RC=0` | ✅（本步的价值就是「松约束自己不红」） |
| 15 步二 | `ledger/types.ts` 加第八个 kind | `TS2741 … 'fabricated' is missing`，`highTier.ts(18,14)` | ✅ |
| 7a | `lenientRead.ts:64` | `names a MIDDLE bad line …` ＋ `marks a torn LAST line torn …` | ⚠️ 计划只写 1 条 |
| 7b | `lenientRead.ts:71` | `a JSON-valid line that is not a correction …` ＋ `reports a bare null line …` | ⚠️ 计划只写 1 条 |
| 11 | `discover.ts:143` | `the integrity gate refuses …`，**恰好 1 条** | ✅ |
| 12 | `discover.ts:80–90`（删 11 行） | `refuses when one projectKey maps to two paths …`，**恰好 1 条** | ✅ |
| 13a | `discover.ts:68–69` → `throw error` | `skips a repo it cannot key …` ＋ `a repo with no remote at all …` | ⚠️ 计划只写 1 条 |
| 13b | 同上 → `continue` | 上述两条 | ✅ |
| 8 | `resolve.ts:78` | `reports a correction whose decision is outside …`，**恰好 1 条** | ✅ |
| 5a | `collect.ts:184` 去掉 mode 判 | `filters rows newer than --as-of …`，**恰好 1 条** | ✅ |
| 5b | 同上 → `if (false)` | `WITHOUT --as-of, a future row is a named refusal …`，**恰好 1 条** | ✅ |
| 18 | `types.ts` ×2 ＋ `compute.ts` ×1 | `carries no scan-duration field …` ＋ golden 逐字节红 | ✅ |

**收尾现测**：`VERIFY_RC=0`，**81/459** 与 **51/167 两个数都未变**；`porcelain -z` 在跑完全部变异后仍是 **0 字节**；`ls ~/.orca` 仍不存在。

## 四、🔴 三条**必须带给下一轮**的实测（别重新发现）

### 1. *** 「期望红在 X」写得【不全】，和写得【假】是同一类危害 ***

上一轮买到的记法防的是**假**预言（点名的断言到不了）。**本轮暴露了同一形状的第二种：真但不全。**
7a／7b／13a 三条各红了两条判据，而计划各只写了一条。成因是同一个：
*** **被删掉的那一行是【多条路径共用的收口】** *** —— `bad(…)` 同时服务「中间坏行」与「截断末行」；
`else bad(parsed.reason)` 同时服务「schema 不合法」与「裸 `null` 行」；`unkeyable.push` 同时服务两种 `CorrectRejection`。
**危险在于**：纪律是「红在别处 ＝ 假红」，看到「说红 1 条、实际红 2 条」最省事的读法就是判它假红，**然后回去改没坏的代码**。
⇒ **记法补一维，两条一起用**：
1. （原有）问「在 X 之前有没有别的断言会先炸」—— 防**假**。
2. 🆕 *** **问「被删掉的那一行，还有【谁】在走它」** *** —— 防**不全**。**写成「红在 X 且仅 X」或「红在 X 与 Y」，不留「至少」。**

⚠️ *** **它与「被删掉的那件事是不是【唯一】在做它的」是同一个问句的两个方向** *** ——
那条问的是共用会不会让变异**变绿**（守卫冗余 ⇒ 静默），本条问的是共用会不会让它**红得比写的多**。
**一次「谁还在走这条路」的普查，同时回答两个问题。**

### 2. *** 守着 C1 的那个扫描器自己带着假阳性被交付了 ***

把计划 Self-Review 第 4 项的 Q1 扫描器原样跑了一遍（dogfood），它报一条 **`BAD`，而那一行是对的** ——
`/bin/rm` ✓、两个实参各带 `:?` ✓、两个变量都在同块 `mktemp -d` 赋值 ✓。
**错的是扫描器**：它把变量名写死成了 `T`，任何不叫 `T` 的正确写法都会被报。
⚠️ 这与「扫描器不许对自己的警告文字报警」是**同一族**，后果一样：*** **读的人学会忽略它，而一个被忽略的护栏等于没有护栏。** ***
而它恰恰是为 C1（四行 `/bin/rm -rf .`）立的最后一道。
⇒ **修正版按【护栏本身】判、不认变量名**，已写进 ERRATUM 2 §五，现测全文 `BAD_COUNT = 0`。

### 3. *** 写那个修正版时，它自己又错了两次 —— 两次都是【反向对照】抓住的 ***

1. 第一版仍报同一行：`ASSIGN` 只认**行首**赋值，而那块写的是 `STORE=…; ROOT=…`（**同一行两个**）。
2. 第二版把**所有**块都报了：补 `[;&|]` 时**漏了 `re.M`**，`^` 于是只匹配整块开头。

⇒ 🔴 *** **记法（从 ccmem 借来，本仓库第一次兑现）：一个扫描器只在语料上跑是不够的，
必须同时有【必抓】和【必不抓】两组样本。** ***
*** **「在本文上跑出 BAD_COUNT = 0」什么都不证明 —— 一个恒返回 0 的扫描器给同样的输出。** ***
本轮那四组对照（C1 原形／有护栏但没赋值／不是 `/bin/rm`／完全正确）写在 ERRATUM 2 §五的表里，**下一位直接抄**。

## 五、本轮**没有**做的（登记，不掩饰）

- **生产代码零改动、判据零改动**；`src/**`、`tests/**`、`scripts/**`、`.decisions/**` 一个字节没动。
- **没有写台账 decision**：本轮两条发现都是**文档缺陷**，不是有备选项的设计裁断
  （对比上一轮 `/6`，那条改了生产代码）。按 A′ 与计划的先例，方法论记法的家是 ERRATUM ＋ handoff。
  *** **登记此判断，人可以推翻。** ***
- **没有写 `overturned`** —— 仍然需要一个 correctionId，而 correction 是**人**的日志，agent 不许替人编。
- **未 push**（控制器一次都没 push）；未建分支、未合并、未删任何分支或 worktree。
- **E3 未开** —— 它的开工前置（correction seam 改既有生产代码）**本轮人未给具名授权**，我没有把「同意按你建议的顺序执行」当成它。
- **裁决甲的 `plan` 那一半没做**（仍需人指名授权改 `preflightUnreadableRepo.test.ts`）。
- **ccloop 的 E1 的 I-2 ＋ 人裁 85 没动**（人裁 121 仍有效）。
...
| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | **E3（面板）** | ⚠️ **开工前置未解**：E3 §9 第 6 项的 correction seam 是**改既有生产代码**，*** 需人另拿一次具名授权 ***。**先要这个，再动手。** |
| **2** | 子系统 **D**，或 B 的后续 | 未变 |
| **3** | 裁决甲的 `plan` 那一半 | 仍需人**指名授权** |
| **4** | ccloop 的 **E1 的 I-2 ＋ 人裁 85** | 人裁 121 仍有效 |

⚠️ **上一轮那张表的第 1、2 条（等 push 授权、补跑 8 条变异）本轮已消解，不要再排它们。**

### 6.1 下一轮开工必读

1. **本节第四条那三条实测** —— 尤其第 1 条，它是对上一轮那条「预言会假」记法的**补维**，两条要一起用。
2. **ERRATUM 2 §五那个修正版扫描器 ＋ 它的四组对照** —— 下一份计划的 Self-Review 直接抄那一版，别抄正文里的旧版。
3. `.decisions/orca-dev-3738db1c.jsonl` **仍是 6 条，本轮没加**。`/4`、`/5` 是对 `/1`、`/2` 的更正，`/6` 是跑变异跑出来的。
4. **开工三条照跑**：`git ls-remote origin refs/heads/main`、`rtk proxy npm run verify`（重定向读回，不过滤）、`ls ~/.orca`（必须不存在）。

## 七、成本

**只抄钩子报出来的数**（Rule 14）：本轮钩子多次报出的都是 *** **约 $9.62** ***。
**此后没有再报过别的数，因此不写 —— 不许自估。** 本轮**没有外派任何评审席**。

### 八、姊妹仓库本轮的同批动作（**已完成；知情，不是本仓库的任务**）

两边都是**就地滚动更新那一节，没有新增章节**（规矩是人 2026-09-02 定的）。**两边都未 push。**

| 仓库 | 做了什么 | 「没碰别人东西」的证明 |
|---|---|---|
| **ccloop** | Orca 那节整节重写，§四加实测 **10、11**（预言写不全／扫描器假阳性）；§三.6 记 E2 变异收口 | 节外 **1–313 行 sha256 前后逐字相同**；13 个 hunk 最早在 **344** 行 |
| **ccmem** | §15 整节重写，「可能有用的实测」加 **13、14**（同两条，措辞对齐本仓库 ⅩⅩⅩⅥ.2.1） | 节外 **1–7549 行 sha256 前后逐字相同**；18 个 hunk 最早在 **7550** 行 |

⚠️ 🔴 *** **ccmem 写入时有【别的线未推的 3 笔】**（ⅩⅬⅠ 那条线：`docs(handoff): roll XLI forward…` 等）。 ***
**一个字都没碰，也没 push。** 现测它们改到那份 handoff 的最远一行是 **7539**，而 §15 起于 **7550** ⇒ **不重叠**。
**ccloop 写入时在 `main`、工作树 0 字节、与远端同点。**

⚠️ **ccmem §15 里那两个「`ev` 计数仍是 0」没有本轮重测，本轮也没给它背书** —— 已在那一节写明。

---

# 📌 本轮续（2026-09-10，会话 `30fa920f`）—— **E3 的开工前置解了：correction seam 已落地**

**归属**：run `orca-dev-30fa920f`。本节**只追加**，上面一字未动。

## 一句话状态

*** **人 2026-09-10 具名授权「correction seam 改生产代码」（E3 spec §9 第 6 项）** *** ⇒ seam 已实施：
`src/corrections/record.ts` 的 `recordNewCorrection`（不碰 argv／stdout），
＋ `correct.ts` 里 `correctionRowFrom()` 把**两处重复的行字面量收成一处**。
**四条变异各自见红。台账 `.decisions/orca-dev-30fa920f.jsonl` 落 1 条。未 push。**
⚠️ **E3 本体（面板、express、web workspace、reviews 表）一行未写** —— 只解了前置。

## 一、⚠️ 远端在会话中途又被推动了 —— 本会话第【三】次

开工时现测「6 笔未推」为假（人已推）；本轮收尾前再测，远端已到 `docs(handoff): record the round that closed out E2's mutations…`。
*** **同一条记法在一个会话里兑现三次，别再当它是偶发。** ***

## 二、做出来的东西

| 买到的东西 | 提交主题行 |
|---|---|
| seam ＋ 字面量收拢 ＋ 7 条判据 ＋ 1 条台账（**275 增 25 删，4 文件**） | `feat(corrections): give the panel and the CLI one place to build a correction` |

## 三、🔴 一条**跑出来的真缺陷**（不是读出来的）

*** **`orca correct --chose-instead ""` 今天就能从真 CLI 走到一个裸 `ZodError`。** ***
`ZodError` 没有 `code` ⇒ `cli.ts:352` 落到 **exit 3「未预料异常」**，把一坨 Zod dump 打给一个只是敲错了一个参数的人。
**§2.3 把它写成「面板将来会造的危害」，实测它今天就在 CLI 上。**
配套现测：`chose_instead: ""` 与省略派生出**两个不同的 id**（`c_407cfe6e…` vs `c_ead8e04b…`）
—— 那正是 store 去重键失效的机制。
⇒ seam **具名拒绝**（`correction-row-invalid`），**不静默归一化**：归一化会让把输入框留空的人以为自己说了话，
而被悄悄丢掉的恰是 A′ §4.3 说全表最值钱的那个字段（Rule 12）。**理由与两条被否掉的备选进了台账 `/1`。**

## 四、🔴 三条**必须带给下一轮**的实测

### 1. *** 我自己犯了本轮上半场刚写下的那条错 ***

M-C（seam 写死 `again: true`）我预言**红 2 条**，实际**红 3 条** ——
第三条是 `close.test.ts` 的 **E19（取锁之后的预期拒绝仍要释放锁）**：`again: true` 让那个拒绝不再发生。
*** **我漏做的正是自己刚补进记法的那一问：「还有谁在走 `again: false` 这条路」。** ***
⇒ **知道一条记法不等于会用它** —— 这是 `orca-dev-d5eb8fa6` 那一轮记过的形状，本轮又撞一次。
**普查要写成命令（`grep` 出所有读该字段／走该分支的判据），不许靠脑补。**

### 2. *** 变异普查会替你发现【缺判据】，而不只是【坏判据】 ***

为了给「两个模式都读同一处字面量」找落点，我先 `grep` 谁读 `by`，结果发现：
**`close.test.ts` 的 E6 走的是 `--close <id>`（close-existing），根本不经 `correctionRowFrom`**；
而 close-new 那一侧**没有任何既有判据钉住 correction 行的字段值**。
⇒ 我那条「stored id 是 stored row 的派生」**在任何「行与 id 一起改」的变异下都照绿** ——
它钉自洽，不钉内容。**补了一条 `close mode stores the row the person described` 才有落点。**
⇒ *** **记法：当你找不到一条变异的落点时，先别改变异 —— 先问是不是缺judge。** ***

### 3. *** 管道又一次吞掉退出码 ***

写完台账我跑 `npm run ledger -- validate .decisions 2>&1 | tail -5`，读到 `LEDGER_RC=0` ——
**那是 `tail` 的退出码。** Rule 14 白纸黑字写着这一条，我照样踩。
⇒ **验证性跑一律重定向到文件再整份读回，一次例外都不要开。**

## 五、四条变异（**每条都看见红，且核过红在点名处**）

| 变异 | 改什么 | 预言 | 实测 |
|---|---|---|---|
| **M-A** | seam 从**另一个行**派生 id | 恰好 3 条，全在 `recordSeam.test.ts` | ✅ 恰好那 3 条 |
| **M-B** | 删掉 seam 的校验守卫（11 行） | 恰好 2 条 | ✅ 恰好那 2 条，且回来的正是 `ZodError` |
| **M-C** | seam 写死 `again: true` | 2 条 | ⚠️ **3 条**（多一条 E19，见第四节第 1 条） |
| **M-D** | 唯一那处字面量的 `by` 改常量 | 恰好 2 条，两模式各一 | ✅ 恰好如此 —— 这是「一处字面量」的证明 |

**收尾现测**：`VERIFY_RC=0`，全仓 *** **82 / 466** ***（此前 81/459），scheduler **51/167 未变**；
`git status --porcelain` 0 字节；`ls ~/.orca` **仍不存在**。

## 六、本轮**没有**做的（登记，不掩饰）

- **E3 本体一行未写**：面板、express、`web/` workspace、reviews 表、绑定守卫、§7 那条端到端成功判据，**全都没动**。
- *** **E3 spec §8 那条前置仍未做** *** —— 「框架选型（React ＋ express）必须先写进台账」，
  A′ 自己举它当高位决策的典型例，而现测 `.decisions/` 里 `React` **仍是零命中**。**开 E3 本体之前必须先落那一条。**
- **没派任何外派评审**。这一刀是单任务、判据 ＋ 变异自证，**没走 spec→plan→两席那条路**。
  ⚠️ **若你认为这一刀该补一席审，说一声** —— 它改了生产代码。
- **未 push**；未建分支、未合并、未删任何分支或 worktree。
- **ccloop／ccmem 本轮【未再更新】** —— 这一刀是 Orca 内部的，对两边都零影响；
  上半场那两次滚动更新已经把 E2 收口写进去了。**登记此判断。**

## 七、⛔ 下一件事

| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | **落 E3 §8 那条框架决策进台账** | `kind: dependency` ／ `scope: repo`，`alternatives` 至少含「服务端渲染 HTML」。*** 写它本身就是这个选择最便宜的检验 *** |
| **2** | **E3 本体** | 有定稿 spec ⇒ 接 `writing-plans`。⚠️ 它会让本仓库**第一次有构建产物**、根依赖从 1 个变成一棵树（人 2026-09-09 明裁） |
| **3** | 子系统 **D**，或 B 的后续 | 未变 |
| **4** | 裁决甲的 `plan` 那一半 | 仍需人**指名授权** |
| **5** | ccloop 的 **E1 的 I-2 ＋ 人裁 85** | 人裁 121 仍有效 |

## 八、成本

**只抄钩子报出来的数**（Rule 14）：本轮钩子报过 **约 $9.62**（长时间未变）→ 随后跳到 *** **约 $74.94** ***。
**此后没有再报过别的数，因此不写。** 本轮**没有外派任何评审席**。

### 九、本节写下之后又做的两件事（**同会话续写，上面一字未动；本节即为对第六节的具名更正**）

#### 1. 🔴 对第六节最后一条的具名更正：**ccloop 与 ccmem 本轮【已经更新了】**

第六节写着「ccloop／ccmem 本轮**未再更新**……登记此判断」。**人随后指示要更新，所以那句现在是假的。**
原文按 Rule 13 逐字保留在上面，这里记更正。

两边都是**就地定点编辑，不是整节重写，且一个编号项都没有新增** ——
规矩是人 2026-09-02 定的「关于 Orca 的章节不能无限增加下去」，本次按它办：

| 仓库 | 怎么改的 | 「没碰别人东西」的证明 |
|---|---|---|
| **ccloop** | 两条新记法**折进已有的第 7、10 条**；§五 那行「开工前置至今未拿到」改成「已授权且已落地，但还有第二条前置」 | 节外 **1–313 行 sha256 前后逐字相同**；4 个 hunk 最早在 **409** 行；全文 470 → **479** 行 |
| **ccmem** | 同两条**折进已有的第 10、13 条**；另加一条**与本仓库真有关**的：seam 会**具名拒绝**空的可选字段，别送空串要省略 | 节外 **1–7564 行 sha256 前后逐字相同**；4 个 hunk 最早在 **7600** 行；全文 7733 → **7746** 行 |

⚠️ 🔴 *** **ccmem 那条 XLI 线本轮【一直在并发干活】。** *** 我提交时它已经比我上半场看到的又多了一笔
（主题行 `docs(handoff): soften the volatile git facts in XLI and add its skills section`），
收尾时它的工作树里还有一个**在飞的判据文件**（`tests/integration/v013-quality-gate-rejects.test.mjs`，7 增 1 删）。
*** **那是别人的东西，一个字节都没碰，也没有 push。** *** 我在那个仓库只改过 `docs/handoff/handoff.md` 一个文件。

#### 2. 关于「别把 git HEAD 写死」（人 2026-09-10 交代）

**本轮新写的 handoff 正文一个 HEAD、一个「领先几笔」都没有写**，指代某一笔一律用**提交主题行**。
⚠️ **唯一一处硬 sha 是有意的**：计划 v2 的 ERRATUM 2 开头写着那 11 次变异「跑在哪一笔上」——
*** **那是【观测锚点】，即那批实测值的有效期，不是当前状态** ***，本节两份姊妹仓库的规矩里都写明这是例外。
它指的那一笔的主题行是 `docs(handoff): record the round that shipped orca metrics, and what the two seats cost`。
⇒ **要判任何一笔发没发布，一律现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`，不许查本文。**
（本会话内远端被人推动了**三次** —— 见本节上文第一条与第七节。）

---

# 📌 本轮（2026-09-10，会话 `3e50f2cb`）—— **E3 计划已落地并经两席外审；开工前置两条全部解除**

**归属**：run `orca-dev-3e50f2cb`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD、不写「领先几笔」** —— 提交本文就会改这两个数，人也会自己推远端。
**要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`。**

## 一句话状态

*** **E3 的实施计划已写完、经两席外派评审、发现全部处置完毕。本体仍一行未写。** ***
两席**都判 `Ready to implement? No`**，合计 **8 条 Critical 全部已修**，10 条登记不修（**其中 2 条需人拍板**）。
另修掉 seam 的一条真缺陷（外审发现）。**未 push、未建分支、未合并、未删任何分支或 worktree。**

## 一、🔴 开工核对推翻了交接给我的「下一件事」

上一节「⛔ 下一件事」第 1 条是「落 E3 §8 那条框架决策进台账」。**它早就做完了。**

现测：`/usr/bin/grep -rc "React" .decisions/` → `orca-dev-ad1e30c6.jsonl:1`。
那一行就是 `orca-dev-ad1e30c6/1`：`kind: dependency` ／ `scope: repo`，
`alternatives` 含「node:http 直接吐服务端渲染 HTML…」并写了 `why_not`，`undo`／`evidence` 齐全；
同文件 `/2` 是 express。落在提交主题行 `feat(decisions): record the two framework choices the spec had left as parentheses`，
且 `merge-base --is-ancestor` 判定**已发布**。

⚠️ *** **那句「React 零命中」是怎么活下来的**：它原文写在 spec §8 里（写下时为真），
随后**被抄进了这条决策自己的 `evidence` 数组**，再被 09-10 两轮 handoff 与 ccmem §15 一路转述，
**没有任何一轮重测过**。 ***
⇒ 🔴 **记法**：*** **一条「现测 X 为零」的断言，一旦被写进【那件让它不再为零的产物】里，就会永久自证。**
引用任何「零命中／不存在」之前，现跑一次。 ***

## 二、做出来的东西（**按提交主题行找，别数笔数**）

| 买到的东西 | 提交主题行 |
|---|---|
| E3 实施计划，9 个 Task ／ 16 条点名变异（2716 行） | `docs(plan): turn the panel spec into nine tasks, each with a mutation that can land` |
| seam 的真缺陷修复 ＋ 1 条判据 ＋ 1 条变异 ＋ 一条具名 ERRATUM | `fix(corrections): stop the refusal from telling a not_my_taste row to leave the field out` |
| 两席外审的全部处置（8 Critical 就地修、10 条登记） | `docs(plan): dispose of what two review seats measured wrong in the panel plan` |

**材料**：计划 `docs/superpowers/plans/2026-09-10-panel-e3.md`（末尾「📌 第一轮外派评审的处置」一节是全部结论，**不在此复述**）。

## 三、实测数（**只抄工具打印的**）

| 项 | 开工 | 收尾 |
|---|---|---|
| `rtk proxy npm run verify` | `VERIFY_RC=0`，全仓 **82 / 466** | `VERIFY_RC=0`，全仓 *** **82 / 467** ***（新判据 +1） |
| 同上 scheduler 档 | **51 / 167** | **51 / 167**（未变） |
| `git status --porcelain -z` | 0 字节 | 0 字节 |
| `ls ~/.orca` | 不存在 | *** **仍然不存在** ***（Rule 17 的活证据） |

⚠️ *** **两个判据数别混着比：82/467 是全仓，51/167 是 scheduler。** *** 历轮有评审员在这里栽过。

## 四、两席的用量与分工（**只抄派出方工具报数；两席自陈都拿不到**）

| 席 | 审什么 | 判定 | 用量 |
|---|---|---|---|
| **seam 那席** | 已提交的生产代码改动（`05277bc`） | `With fixes` —— 0 Critical ／ 1 Important ／ 4 Minor | **151,642 token ／ 51 次调用 ／ 1,105,326 ms** |
| **计划第一席** | 把计划自己写的话现测成假的 | `No` —— 4 Critical ／ 8 Important ／ 7 Minor | **199,931 token ／ 41 次调用 ／ 836,181 ms** |
| **计划第二席** | **对撞**（各自正确的东西两两相撞） | `No` —— 4 Critical ／ 9 Important ／ 2 Minor | **176,495 token ／ 28 次调用 ／ 676,344 ms** |

⇒ *** **一席稳定在 150k–200k token，连续五轮成立。** ***
⇒ *** **分工是对的**：两席交叉印证 9 条，各自独有 4 条，而第二席那 4 条独有发现**没有一条**是「审计划写的话」能抓到的。 ***
⚠️ **派第二席时必须指名要它做对撞** —— 这是它独有的产出，E2 那一轮已经验证过，本轮第二次兑现。

## 五、🔴 四条**必须带给下一轮**的实测

### 1. *** 8 条 Critical 里，没有一条是「设计错了」 ***

**全部是「判据是空的／跑不动／落点错了」。** 逐条形状：
判据的期望值由**被测函数自己**算出（深相等恒真）；断言比的是**被变异的那个常量**（两边一起动）；
夹具由 `mkdtemp` 建好 ⇒ 钉的是 `mkdtemp` 不是程序；`at` 进 id 派生且不可注入 ⇒ 判据**无变异时也永远红**；
路由参数对不上真实数据形状 ⇒ 红的是「没匹配」不是被钉的行为。

⇒ 🔴 *** **spec 层的自审已经足够，判据层的自审远远不够** —— 而判据层在计划阶段【根本没法真做】，
因为被测代码还不存在。 ***
⇒ **下一轮考虑：计划评审只买「结构与落点」，「红数」的验收放到实施之后。** **本轮最贵的一课。**

### 2. *** 16 条「红在 X 且仅 X」我写错了 5 条 —— 错误率 31%，而且普查写成 grep 也没兜住 ***

错的是 F-4b（3→2）、F-4（1→2）、L-3（1→2）、P-2（落点整个搞反）、C-13（落点作废）。
⚠️ **F-4b 那条尤其值得记**：我以为两条 caveat 是同一样东西，
*** **grep 搜 `caveat` 会同时命中两处、看起来正好印证我的预言** *** —— 而它们来自**两个不同的字段**。
⇒ 🔴 **记法补一维（第三维了）**：问「X 之前有没有别的断言先炸」（防假）＋「被删那行还有谁在走」（防不全）
＋ 🆕 *** **「点名那条断言里的【字面量】，是从哪个字段来的？」** *** —— 两条长得像的字符串，来源可能不同。

### 3. *** 把危害从变异列里清干净了，它会从【判据的夹具形状】里漏回来 ***

spec 外审 I5 曾把 `0.0.0.0` 从变异列里清掉。本轮计划照做了 —— **然后写了一条省略 `bind` 的判据**，
而 `listen(port, undefined)` **绑所有网卡**。⇒ 跑删掉 bind 守卫那条变异时，它会**真的把面板发布出去**。
⇒ *** **「这条判据在守卫被删掉的那次跑里会做什么」要连【夹具的默认值展开成什么】一起问。** ***

### 4. *** 远端在一个会话里被推动【四】次 ***

开工时 3 笔未推；中途一次；收尾前现测**又一次**，那 3 笔（含 seam）全部变成已发布文本
⇒ 本轮对 seam 注释的更正**只能追加 ERRATUM**。
⚠️ **这条记法在本仓库连续三轮兑现，别再当它是偶发。** **每次「就地改」之前现跑，开工那次的结果不能复用。**

## 六、本轮**没有**做的（登记，不掩饰）

- *** **E3 本体一行未写** *** —— `web/`、`src/panel/`、express、reviews 表、面板判据，**全都不存在**。
  现测 `ls src/panel/` → not found；`ls -d web` → not found。
- **计划的 10 条登记项未做**，其中**两条需人拍板**：
  1. 🔴 *** **把 `at`／`projectKey` 的戳法搬进 seam 并做成可注入** *** —— 这是**改既有生产代码，需另拿具名授权**。
     不做的话，spec §2.3 加粗那条判据（「面板与 CLI 同一份输入产出同一个 correction id」）**今天做不到**，
     本轮用的是降级判据（钉 seam 专属的具名拒绝）。**这是一处已知降级。**
  2. 🔴 *** **spec §3.1 有一处未登记的错误** *** —— 它写「`schema.ts` 拒绝为 `wrong`/`stale` 强制 `chose_instead`」，
     现测强制的是 **`not_my_taste`**。不在 spec §12 那六条里，**欠一条具名 ERRATUM**。
- **未 push**（控制器一次都没 push）；未建分支、未合并、未删任何分支或 worktree。
- **裁决甲的 `plan` 那一半没做**（仍需人指名授权改 `preflightUnreadableRepo.test.ts`）。
- **ccloop 的 E1 的 I-2 ＋ 人裁 85 没动**（人裁 121 仍有效）。
- **没有为 seam 的修复再派一席**（那一刀只加了一条判据 ＋ 一条变异，且变异见红）。**登记此判断，人可以推翻。**

## 七、⛔ 下一件事

| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | **人拍板那两条**（`at` 可注入需具名授权；spec §3.1 的 ERRATUM） | 都在计划的「登记而不修」一节，**别自己开** |
| **2** | **E3 本体**：接 `superpowers:subagent-driven-development`，逐 Task 做 | ⚠️ **Task 5 有三处「实施时定死一种」**：`as_of` 注入、`LIST_FIELDS` 跨 workspace、`it.skip` 闸门。**别绕过** |
| **3** | 子系统 **D**，或 B 的后续 | 未变 |
| **4** | 裁决甲的 `plan` 那一半 | 仍需人**指名授权** |
| **5** | ccloop 的 **E1 的 I-2 ＋ 人裁 85** | 人裁 121 仍有效 |
...
1. **计划末尾「📌 第一轮外派评审的处置」整节** —— 8 Critical 的修法、5 条错的红数、10 条登记项。
2. **本节第五条那四条实测**，尤其第 1 条（判据层自审在计划阶段做不了）与第 2 条（记法的第三维）。
3. **开工三条照跑**：`git ls-remote origin refs/heads/main`、`rtk proxy npm run verify`（重定向读回，不过滤）、`ls ~/.orca`（必须不存在）。
4. ⚠️ **别信任何「零命中／不存在」的转述**（见本节第一条）——**现跑。**

## 八、成本

**只抄钩子报出来的数**（Rule 14）：本轮钩子报过 **约 $14.91** → *** **约 $93.16** ***。
**此后没有再报过别的数，因此不写 —— 不许自估。**
⚠️ **三席外派是大头**（合计约 528k token）。**大动作前先跟人报预估，这一轮是人先点的头。**
...
# 📌 本轮（2026-09-16，会话 `cbe8dda3`）—— **parked N-1 已修**；R71 点名的另一半经实测判为冗余，未落地

**归属**：run `orca-dev-cbe8dda3`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD，也不把「领先几笔」当【当前状态】写** —— 提交本文就会改这两个数，人也会自己推远端。
**要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`。**
⚠️ *** **唯一例外，与 ccloop 那一节同一条口径：带【观测锚点】的实测值要写** *** —— 那是它的**有效期**，不是当前状态。
下面第零节里那三个笔数就是这样的实测值：它们**当场就过期了**，而那件事本身正是本轮要留给下一位的记法，
**不是一份仍然有效的状态描述**。**任何时候都别拿它们当现状，现跑 `ls-remote`。**

## 一句话状态

*** **E3 唯一那条 parked（N-1）修完了：切换决策时先把旧详情清掉，并新增一道守卫丢弃过期应答。面板就 N-1 这一项而言可以碰真数据了。** ***
面板**仍然只记、不闭环**。未 push、未建分支、未合并、未删任何分支或 worktree。**只做了这一件事就停。**

## 零、开工核对（都带命令，本轮现测）

| 项 | 开工 | 收尾 | 命令 |
|---|---|---|---|
| 全仓 npm test | 95 / 561 | **95 / 561**（未变） | `rtk proxy npm run verify > 文件 2>&1` |
| scheduler 档 | 51 / 167 | **51 / 167**（未变） | 同上 |
| verify:panel | PASS 0–12 | **PASS 0–12**（未变） | 同上 |
| web check 档 | 6 / 18 | *** **8 / 26** *** | 同上 |
| `VERIFY_RC` | 0 | **0** | 同上 |
| `git status --porcelain -z` | 0 字节 | 0 字节（提交后） | **必须 `/usr/bin/git`** |
| `ls ~/.orca` | 不存在 | *** **仍不存在** ***（每条变异前后各复核） | |

✅ *** **开工那次 verify（1400 行）控制器【整份读完】了** *** —— 上一轮登记的「只用 python 索引汇总行」本轮没有重演。
⚠️ **开工现测：三个仓库都有未推的本地提交**，且每个的远端都是本地的祖先（`merge-base --is-ancestor` RC 0）：Orca 15 笔、ccloop 1 笔、ccmem 1 笔。
🔴 *** **写收尾文档时再量一次，三个数全变了** *** —— 人在会话中途把三个仓库都推了：ccloop 与 ccmem 变成同点，
Orca 的远端走到了开工时的本地 HEAD 上，于是本轮只剩**这一笔修复**未推。
⚠️ **这一次是被当场抓住的**：控制器往 ccmem 写「本地领先 1 笔」的那一行，写完复核时已经为假，**在提交之前就地改掉了**。
⇒ *** **「未推」会过期成「已推」，本轮是第二次兑现；开工那次 `ls-remote` 的结果【任何时候都不能复用】。** ***

## 一、人本轮拍的

- **H7**（AskUserQuestion）：N-1 的判据**两条都做** —— 一条钉守卫本身的纯判据，加一条真正驱动 `App` 的 jsdom 交互判据。
  控制器**先量后问**：现有设施（`environment: "node"` ＋ `renderToStaticMarkup`，而 `useEffect` 在静态渲染里根本不跑）下，这个 bug **结构上不可能红**；问题里把 jsdom 那一半的依赖代价一并摆出来了。

## 二、做出来的东西（**按提交主题行找，别数笔数**）

| 买到的东西 | 提交主题行 |
|---|---|
| 切换选中行时先清旧详情；`acceptArrival` 丢弃过期应答；4 条 jsdom 交互判据 ＋ 4 条纯判据；web 档加 jsdom 与 @testing-library/react | `fix(web): take the previous decision off the screen before loading the next` |

**材料**：SDD 台账 `.superpowers/sdd/2026-09-10-panel-e3/progress.md` 的「Session 5」一节（裁决 **R72**，末尾有 NEXT）；
台账行 `.decisions/orca-dev-cbe8dda3.jsonl` 第 1 行。

## 三、🔴 本轮最值钱的两条（**下一轮直接用**）

1. *** **一条 parked 裁决点名的两件事，可以有一件是冗余的 —— 而这只能量，不能读。** ***
   R71 同时点名「清空」与「给 `DecisionDetail` 上 `key`」。本轮先做清空，再**直接量** key 还有没有事可做：
   写一个探针，让决策的 fetch 是**已决议的 Promise**（React 最可能把清空与赋值批进同一次提交的那种情况），
   切换后取 textarea —— 现测 *** **`PROBE same-node: false value: ""`** ***，即组件每次切换都真的卸载了，非受控表单的 DOM 本来就会重建。
   ⇒ **`key` 在我能走到的每条路径上都是冗余的，没落地**（裁决 R72，undo 是一行 JSX，已记进台账行）。
   ⚠️ 这与本文件早已记过的「冗余守卫让变异静默」是同一条的**正面用法**：
   *** **不是「发现变异红不了才去查」，而是「落地之前先问一句：删掉它自己的那条变异，能红吗？」** *** 答不上来就去量。
2. *** **写判据时会撞见 parked note 没点名的第二个洞 —— 因为判据逼你把时序写出来。** ***
   R71 只说了「切换时不清详情」。写第 2 条判据（两个 fetch 都在飞，B 先回、A 后回）时才看见：
   **根本没有任何东西保证应答按请求顺序到达**，慢的那个会在人已经走开之后把页面重画回去。
   ⇒ 同一笔一起关掉了。⚠️ **记法**：*** **给一个 UI 竞态写判据时，把「两个都在飞」单独列成一条 —— 只测「切换瞬间」测不到乱序。** ***

## 四、变异（**本轮由控制器在 `git clone --local` 副本里跑**，副本 `node_modules` 走符号链接，四个工作树文件拷进去并逐字节比对为 0）

**四条全部落地（sha 前后不同）、全部见红、且四条「红在 X 且仅 X」的预言【条条精确命中】，一条都没有「至少」。**

| 变异 | 预言与实测（一致） |
|---|---|
| **M-1** 删掉 effect 里的 `setDecision(null)` | appSelection 第 1 条（旧详情仍在屏上）与第 3 条（半截的理由带过去了），且仅此两条 |
| **M-2** `acceptArrival` 恒 `return true` | selection 第 2、3、4 条 ＋ appSelection 第 2 条，且仅此四条 |
| **M-3** `acceptArrival` 改用 `projectKey + "::" + id` 定长分隔符拼接 | selection 第 4 条（碰撞那条）一条 |
| **M-4** `onCorrect` 用 `home.todo[0]` 而不是 `selected` 造 body | appSelection 第 4 条一条 |

⚠️ **如实登记**：appSelection 第 4 条（「纠正打到屏幕上那条决策」）**在修之前就是绿的** —— 它是回归守卫，
**能不能红是靠 M-4 证明的，不是靠它自己**。修之前见红的是另外三条，且**每一条都红在它自己点名的那句断言上，没有一条是模块加载失败**。

## 五、本轮**没有**做的（登记，不掩饰）

- 挂账一条没动：`reviews.jsonl` 留存策略、多进程重复 review 行无判据、**`npm install` 那 5 个漏洞仍未分诊（R17）**、N-3、N-4、终审判 fine-to-leave 的 9 条 Minor。
- **依赖如实报**：web 档新增 `jsdom` 与 `@testing-library/react`（50 个包）。
  *** **`npm audit` 前后都现测过，总数【未变】：5（3 moderate／1 high／1 critical），全在既有的 vitest／vite／esbuild 那条链上。** *** 新依赖没有带进新漏洞。
- 子系统 D、B 的后续、裁决甲的 `plan` 那一半、ccloop 的 E1 的 I-2 ＋ 人裁 85 都没动。
- **未 push**（控制器一次都没 push）。
...
| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | 挂账里挑：`reviews.jsonl` 留存策略、5 个漏洞分诊（R17）、N-3／N-4 | 都在 SDD 台账 Session 5 的 NEXT 里 |
| **2** | 子系统 **D**，或 B 的后续；裁决甲的 `plan` 那一半（需人指名）；ccloop 的 E1 的 I-2 ＋ 人裁 85 | 未变 |

**开工三条照跑**：`/usr/bin/git ls-remote origin refs/heads/main`；`rtk proxy npm run verify` 重定向读回
（期望 **95/561、51/167、verify:panel PASS 0–12、web 8/26**，整条约 40 多秒）；`ls ~/.orca` **必须不存在**。
⚠️ `npm run verify` 会**起真进程、绑回环端口、写临时目录**（都会自己清掉）—— 看到 `orca panel` 进程一闪而过是正常的。

## 七、成本与用量

**只抄工具报出来的数**（Rule 14）：**本会话的钩子一次都没报过数，因此不写。** 本轮**没有外派任何 subagent**，
控制器自己跑了 2 次全量 verify ＋ 1 次探针 ＋ 1 次副本基线 ＋ 4 条变异单跑。
⚠️ **控制器自己的上下文占用没有工具报数，不写结论。**

*** ERRATUM（同会话，写完本节之后，run `orca-dev-cbe8dda3`）***：上面那句「钩子一次都没报过数」**写下时为真，此后为假** ——
钩子在收尾阶段报了一次 *** **约 $72.87** ***（会话累计，工具报数，不是控制器估算）。原文逐字保留。
⚠️ **这是本轮第二次兑现同一条**：*** **一句「现测 X 不存在／没发生」写进文档的那一刻就开始过期，而它不会自己更新。** ***
第一次是三个仓库的「未推」笔数（见第零节）。**引用任何这类句子之前，去它所描述的那个来源现看一眼。**

## 八、姊妹仓库本轮的同批动作（**已完成；知情，不是本仓库的任务**）

两边都是**就地滚动更新那一节，没有新增编号项**（规矩是人 2026-09-02 定的）；**两边都未 push**。
- **ccloop**：「📌 Orca 那条线」§五 的 E3 一条改为「parked N-1 已修」；「冗余守卫」那一条（第 7 条）补上本轮的**正面用法**。
- **ccmem**：§15 的 E3 一条同样改写；同一条实测折进已有的第 10 条。

---
# 📌 本轮（2026-09-16，会话 `4ccc0a0b`）—— **挂账清掉三条：R17 分诊、N-3、N-4**；N-4 的一半经实测判为冗余，未落地

**归属**：run `orca-dev-4ccc0a0b`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD，也不把「领先几笔」当【当前状态】写**。**要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote` ＋ `git merge-base --is-ancestor`。**
⚠️ 唯一例外同前几节：**带【观测锚点】的实测值要写** —— 那是它的有效期，不是当前状态。

## 一句话状态

*** **E3 的三条登记挂账清掉了：R17（5 个依赖漏洞）以「不升级 ＋ 一条离线机械判据」收口，N-3（413/415 被压平成 400）与 N-4（`[localhost]` 被当成合法 Host）各自修掉。** ***
面板**仍然只记、不闭环**。未 push、未建分支、未合并、未删任何分支或 worktree。**人点名的三件，一件不多一件不少。**

## 零、开工核对（都带命令，本轮现测）

| 项 | 开工 | 收尾 | 命令 |
|---|---|---|---|
| 全仓 npm test | 95 / 561 | *** **95 / 566** ***（+5） | `rtk proxy npm run verify > 文件 2>&1` |
| scheduler 档 | 51 / 167 | 51 / 167（未变） | 同上 |
| verify:panel | PASS 0–12 | PASS 0–12（未变） | 同上 |
| web check 档 | 8 / 26 | 8 / 26（未变） | 同上 |
| `VERIFY_RC` | 0 | **0** | 同上 |
| `git status --porcelain` | 空 | 0 字节（提交后） | **必须 `/usr/bin/git`** |
| `ls ~/.orca` | 不存在 | **仍不存在**（每条变异前后各复核） | |

🔴 *** **上一轮收尾写着「三个仓库都有未推的本地提交」（Orca 15、ccloop 1、ccmem 1）。本轮开工现测：三个全部与远端同点。** ***
人在两轮之间把三个都推了。⇒ **这是同一条记法第三次兑现**，而且它**当场改变了本轮的动作**：
本轮要碰的两个注释块因此是**已发布文本** ⇒ 原文逐字保留 ＋ **追加**具名 ERRATUM，一处都不许就地改。

## 一、人本轮拍的

- **（AskUserQuestion，2026-09-16）**：本轮做 **R17 ＋ N-3 ＋ N-4** 三件小的；`reviews.jsonl` 留存策略与子系统 D **各自另开一轮**。
  控制器在问之前已经把 R17 用 `npm audit --json` 量完了，把「五条全在 devDep 链上」和「修它要 vitest 大版本跳」一起摆在了选项里。

## 二、做出来的东西（**按提交主题行找，别数笔数**）

| 买到的东西 | 提交主题行 |
|---|---|
| N-3：body-parser 的状态码透传；N-4：括号里必须含冒号才算 IP-literal；R17：一条离线机械判据 ＋ 必抓样本；两处具名 ERRATUM | `fix(panel): let the body parser's own status reach the client, and read brackets as an IP literal` |

**材料**：SDD 台账 `.superpowers/sdd/2026-09-10-panel-e3/progress.md` 的「Session 6」一节（裁决 **R73／R74**，末尾有 NEXT）；
台账行 `.decisions/orca-dev-4ccc0a0b.jsonl` 第 1、2 行。

## 三、🔴 本轮最值钱的三条（**下一轮直接用**）

1. *** **R72 那条记法第二次用，而这次答案是「红不了」—— 于是那半个修法没落地。** ***
   N-4 一开始写了**两道**守卫：括号内容限定为十六进制字符/冒号/点的字符类，加上「必须含冒号」。
   落地前问「删掉字符类【它自己】的变异能红吗」，答不上来 ⇒ 去量：把字符类换成 `[^\]]+`，
   sha `ebf833c990f012c3` → `192de6eadf884df9`（变异行读回确认），**11 条判据全绿**。
   ⇒ 因为 `hostnameOf` 的返回值只会被拿去和 `{127.0.0.1, localhost, ::1, bind}` 做精确匹配，
   **两种畸形写法都不含冒号**，冒号那一道单独就做完了全部的事。⇒ **字符类删掉**（裁决 R74）。
   ⚠️ **记法补一句**：*** **「先问再落地」的代价是一次探针；「落地了再回头查」的代价是一整轮变异。两次兑现，两次都省下了后者。** ***
2. *** **一条「写下来那天就是绿的」守卫判据，必须自带【必抓样本】，否则它和恒返回空的桩没有区别。** ***
   R17 那条判据钉的是不变量（那条链只出现在 devDependencies 与 dev-only 的 lock 条目里），**今天本来就成立**。
   本轮先把谓词写成 `return []` 的桩 —— *** **必抓样本红了，而【真实仓库那条判据照绿】。** ***
   那正是它要防的假绿。⚠️ **这与 ccloop 那份文档第 11 条同源**：
   *** **「在本文上跑出 BAD_COUNT = 0」什么都不证明 —— 一个恒返回 0 的扫描器给同样的输出。** ***
3. 🔴 *** **`npm audit` 的文字报告会把 high 和 critical 折叠掉。** ***
   现测（2026-09-16）：文字报告只列出 **2** 条 advisory（都是 moderate），它自己的汇总行却写 **5 (3 moderate, 1 high, 1 critical)**。
   被折叠的正是 `vite` 的 high 与 `vitest` 的 **critical**。
   ⇒ *** **只读文字报告的人会漏掉最严重的那条。分诊一律走 `--json`。** ***

## 四、变异（**本轮由控制器在 `git clone --local` 副本里跑**，根与 web 的 `node_modules` 走符号链接，五个工作树文件拷进去并逐字节比对为 0）

**六条全部落地（sha 前后不同、变异行读回）、全部见红、六条「红在 X 且仅 X」的预言【条条精确命中】，一条「至少」都没有。**
副本基线 3 files / 35 tests 全绿；副本的 `package.json` 与 `package-lock.json` 每次也还原到 0 字节。

| 变异 | 预言与实测（一致） |
|---|---|
| **M-1** 删掉 `literal.includes(":")` | N-4 那条判据，且仅此一条 |
| **M-2** `res.status(err.status)` → 写死 400 | N-3 的 413 与 415 两条，且仅此两条 |
| **M-3** 删掉 `shippedAuditedPackages` 的 manifest 循环 | 必抓样本一条（`['esbuild']`） |
| **M-4** 删掉它的 lockfile 循环 | 必抓样本一条（`['vitest']`） |
| **M-5** 副本里把 `vitest` 挪进 `dependencies` | **真实仓库那条**判据一条（`['vitest']` vs `[]`） |
| **M-6** 副本 lock 里把 `esbuild` 的 `dev` 翻成 false | **真实仓库那条**判据一条（`['esbuild']` vs `[]`） |

⚠️ **M-5／M-6 才是证明「真实仓库那条判据真的在守」的那两条** —— M-3／M-4 只证明谓词不是空壳。**两组缺一不可。**

## 五、本轮**没有**做的（登记，不掩饰）

- `reviews.jsonl` 留存策略、多进程重复 review 行无判据、R68 那 9 条 fine-to-leave 的 Minor：一条没动。
- **R17 是以「裁决」收口的，不是以「升级」收口的** —— `npm audit fix --force` 会把 vitest 抬到 5.0.1（breaking）。
  将来若要升，**三档基线（95/566、51/167、PASS 0–12）全部要重量**。
- 子系统 D、B 的后续、裁决甲的 `plan` 那一半、ccloop 的 E1 的 I-2 ＋ 人裁 85 都没动。
- **未 push**（控制器一次都没 push）。
- ⚠️ **一处纪律上的如实登记**：收尾那份 1406 行的 verify 日志，控制器用的是**穷尽式分类扫描**
  （每一行都要匹配上一条已知良性模式，**匹配不上就原样打印；实测 0 / 1406 匹配不上**），
  **不是逐行读完**。开工那份 1391 行**是逐行读完的**。
  ⇒ 这比 Session 4 的「只取汇总行」强（没有任何一行被悄悄丢掉），比 Session 5 的逐行读弱。**如实记在这里，不粉饰。**

## 六、⛔ 下一件事

| 顺序 | 做什么 | 说明 |
|---|---|---|
| **1** | `reviews.jsonl` 留存策略 ＋ 多进程重复 review 行 | ⚠️ **它写仓库外的用户全局数据（CLAUDE.md Rule 17）** ⇒ 是政策决定不是补丁：先 `brainstorming` → spec → plan，**判据一律走 `ORCA_CORRECTIONS_DIR` 改道** |
| **2** | 子系统 **D**，或 B 的后续；裁决甲的 `plan` 那一半（需人指名）；ccloop 的 E1 的 I-2 ＋ 人裁 85 | 未变 |

**开工三条照跑**：`/usr/bin/git ls-remote origin refs/heads/main`；`rtk proxy npm run verify` 重定向读回
（期望 **95/566、51/167、verify:panel PASS 0–12、web 8/26**，整条约 40 多秒）；`ls ~/.orca` **必须不存在**。
⚠️ **`95/566` 是本轮之后的新基线，561 作废。**

## 七、成本与用量

**只抄工具报出来的数**（Rule 14）：*** **本会话的钩子到写下本节为止一次都没报过数，因此不写。** ***
⚠️ **上一轮同一句话在同一个会话里变成了假的**（钩子在收尾才报了一次），**所以这句话的有效期只到你读到它的那一秒 —— 去源头现看一眼。**
本轮**没有外派任何 subagent**；控制器自己跑了 3 次全量 verify ＋ 1 次探针 ＋ 1 次副本基线 ＋ 6 条变异单跑 ＋ 若干单文件跑。
⚠️ **控制器自己的上下文占用没有工具报数，不写结论。**

## 八、姊妹仓库本轮的同批动作

**本轮对 ccloop 与 ccmem 的产品代码、判据、脚本零触碰**；只就地更新了各自那一节（详见本节提交之后的那一笔）。

*** ERRATUM（同会话，写完本节之后，run `orca-dev-4ccc0a0b`）***：上面第七节那句「钩子到写下本节为止一次都没报过数」
**写下时为真，此后为假** —— 钩子随后报了一次 *** **约 $64.40** ***（会话累计，工具报数，不是控制器估算）。原文逐字保留。
⚠️ *** **这是同一条记法在【连续两轮】里各兑现一次，而且两轮都栽在同一个位置（成本那一节）。** ***
...
# 📌 本轮（2026-09-17，会话 `18c77f3e`）—— **上一轮 22 条裁决人已全部通过；spec §8 第 11 条（`null` 行让面板崩溃）已修**

**归属**：run `orca-dev-18c77f3e`。本节**只追加**，上面一字未动。
⚠️ **本节不写任何 HEAD，也不写「领先几笔」**。指代某一笔引**提交主题行**；判发布状态**现跑 `git ls-remote` ＋ `merge-base --is-ancestor`**。带观测锚点的实测值照写。

## 一句话状态

*** **`readReviews` 不再把 `null`／数字／数组／键字段非字符串的行当成一行：面板启动、`/api/metrics`、`/api/todo` 都不再因此崩溃。** ***
主题行 `fix(panel): drop a reviews.jsonl line that is not a row instead of crashing on it`（一笔，含 spec ERRATUM 1）。**面板仍然只记、不闭环。**

## 零、开工核对（本轮现测）

| 项 | 开工 | 收尾 | 命令 |
|---|---|---|---|
| npm test | 100 / 606 | *** **100 / 608** *** | `rtk proxy npm run verify > 文件 2>&1`，两次都整份读回（1409 行／1410 行） |
| verify:scheduler | 51 / 167 | 51 / 167 | 同上 |
| verify:panel | PASS 0–13 | PASS 0–13 | 同上 |
| web check | 8 / 26 | 8 / 26 | 同上 |
| `VERIFY_RC` | 0 | 0 | 同上 |
| `ls ~/.orca` | 不存在 | 不存在（verify 前后、变异前后都核） | |
| 三仓库 `ls-remote` | *** **三个都与远端同点、零领先零落后** *** | 见文末 | `/usr/bin/git ls-remote` ＋ `rev-list --count` |

⚠️ 开工 brief 里写着「三个仓库都有未推的提交」—— **现测是假的**，人已全推。⇒ 上一轮的 spec、plan、progress.md、代码注释从此都是**已发布文本**。
⚠️ *** **`100/606` 已作废，新基线 `100/608`。** ***

## 一、人本轮拍的

- **上一轮 progress.md 的 22 条 `Ruling:` 全部通过**（控制器逐组列出并建议通过；人：「同意，继续」）。
- **顺序**：先修 spec §8 第 11 条（本会话），**然后子系统 D，B 的后续暂缓** —— 理由是量出来的：`~/.orca` 不存在 ⇒ 本机没有一条真实 correction，B 要吃的 `not_my_taste` 样本为零，现在设计 B 的判据无法量。
- **长期授权**：「执行中有问题先按你的建议执行，最后阶段报给我审核」。

## 二、做出来的东西（细节不复述，看 spec ERRATUM 1）

**材料**：`docs/superpowers/specs/2026-09-16-reviews-compaction-design.md` 末尾 **ERRATUM 1**（修法、删掉的冗余守卫、C23／C23b、M33／M34 与预言）。
要点只三条：
1. `readReviews` 用 `compactClassify.ts` 的 `parseRow`（改为导出）判「是不是一行」—— 面板与 `orca compact-reviews` **同一个「读不出」定义**。
2. *** **`wantedDecisions` 里的同形过滤修后冗余（旧变异 M30 必绿）⇒ 一并删除**，C22 改由 M33 钉住；已发布注释追加 ERRATUM。 ***
3. 变异在 `clone --local` 副本里只跑 `tests/panel`（副本里调度器判据按相对路径找 ccloop，**先天红**，与上一轮 17/132 基线同因）：
   M33「删掉跳过行」预言 C22、C23、C23b 且仅此三条 —— **精确**；M34「`continue`→`break`」预言 C23、C23b 且仅此两条 —— **精确**。本次无新增 `finally`。

## 三、🔴 本轮值得带走的

1. *** **「加判据时回扫旧预言」有反方向：改生产代码时，回扫旧变异会不会【变绿】。** *** 在上游修根因，会让下游一道旧守卫变冗余、钉它的旧变异失去意义。本轮在落地前查出 M30，代价是一次 grep。
2. *** **注释里写「会答 500」之前先量。** *** 控制器起初是读代码推出来的；补了一个纯函数探针（两个覆盖率函数遇 `null` 都抛 TypeError）＋ 读错误处理器，注释改成只说量到的。**HTTP 层没实际发请求 —— 如实登记。**
3. **开工 brief 本身也会过期**：「三仓库都有未推提交」在开工那一刻已不成立。**brief 里的发布状态同样只能现测。**

## 四、本轮**没有**做的

- spec §8 其余 10 条（含第 9 条「归档证据只看当前分支」）**都没动**。
- 子系统 D、B 的后续、裁决甲的 `plan` 那一半、ccloop 的 E1 I-2 ＋ 人裁 85：**都没动**。
- 本轮没有派任何 subagent、没有外审（改动小；变异两条精确）。**若人要外审，这一笔是独立可审的一笔提交。**
- **未 push、未建分支、未合并、未删任何分支或 worktree。** 变异副本（scratchpad 下）在字节比对后已删。

## 五、⛔ 下一件事

| 顺序 | 做什么 | 说明 |
|---|---|---|
| **0** | **人审本轮**：那一笔 fix 提交 ＋ 三份 handoff 的更新 | 收尾对话里已列 |
| **1** | *** **子系统 D（检查点交接）的设计，建议新开会话** *** | 先 `superpowers:brainstorming`。接口已在主 spec §8：触发器挂**上下文窗口水位**（不是累计消耗）。**第一个要答的问题：Orca 能不能拿到运行中 agent 的窗口占用读数** |
| **2** | B 的后续 | **暂缓**，直到有真实 correction（`~/.orca` 存在且有 `not_my_taste` 行）。⚠️ 那时开工核对里「`~/.orca` 必须不存在」要改成「判据前后快照不变」，需人拍 |
| **3** | 小挂账 | spec §8 第 9 条需要「跨分支归档证据」的设计；裁决甲 `plan` 那一半需人指名 |
...
| skill | 什么时候用 |
|---|---|
| `superpowers:brainstorming` | 开子系统 D 之前（**必走**），终点接 `writing-plans` |
| `superpowers:writing-plans` | D 的 spec 定稿后；写完跑自查三项，并给每个 `finally`／失败路径点名变异 |
| `superpowers:test-driven-development` | 任何修复（本轮就是：先看 C23／C23b 红，再改） |
| `superpowers:verification-before-completion` | 说「绿了／做完了」之前 |
| `superpowers:requesting-code-review` | 派外审前先报预算（只报工具数） |

## 六、成本

**只抄工具报的数**：钩子在本节写作前最后一次报本会话累计 *** **约 $43.61** ***；此后若再报，以钩子为准。本轮无 subagent。
...
## 2026-09-19 公共控制协议、handoff 与 D3 八任务完成（task 01a0b8f9）

本节是新的接手优先入口，取代上节“生产 ccloop 协议仍缺失／先写计划”的下一步。已批准计划 `docs/superpowers/plans/2026-09-19-ccloop-control-handoff-d3.md` 八任务完成；Orca 控制消费端在当前开发树，ccloop 生产端在 `/tmp/ccloop-codex-0919`。两分支仍分别为 `codex/control-foundation-0919` 与 `codex/codex-adapter-0919`，均未整合 main。按提交主题 `test(control): verify real ccloop recovery protocol`、`feat(control): verify cross-repo recovery protocol` 定位，不以本文里的历史 SHA 推断当前 HEAD。

交付契约：ccloop `control` v1 支持 capabilities/accept/inspect/handoff/collect/read-evidence，持久 accepted、严格幂等身份、work/handoff 两桶明确累计观测、机械交接包、两次进程组静止证明、完整脏 Git 快照恢复和独立结果仓库。Orca 生产 port 只接受显式可执行文件/配置，不回退旧 runner；归档并重读全部证据后提交 checkpoint，continuation 领取新 run 且只继承 predecessor grant 减累计用量，D3 task/group JSON 权威并确定性投影 Markdown。Codex 能力仍是 `phase-end + soft`，strict 未实现。

真实二进制离线验收跑通 accept/replay → 三阶段 usage → handoff/proof → archive/commit → dirty bundle → 新 run → collect；第一、continuation execution 各登记三个 fake Codex 进程组。六个同步 SIGKILL 边界恢复不重复启动、计费、checkpoint、continuation 或 D3 Markdown。最终 ccloop `verify:control` 24/306、全套 56/771、typecheck/build RC0；Orca cross-repo 1/3、control 19/120、完整 verify 主套 148/1195 两次、scheduler 51/167、chain 13/213、Web build、panel PASS0–14、Web 9/34，均 RC0且正式日志无 skipped/todo。精确命令、日志 SHA、run/execution/checkpoint/group IDs、Node v24.13.0 和配置哈希见 `.superpowers/sdd/2026-09-19-ccloop-control-handoff-d3/task-8-final-metadata.md`。

一次实际竞态已修：polling 不能创建 terminal source 路径；terminal 在 candidate durable 前不外露；ccloop 负责精确物化 `sourceDir/repo`，Orca 只读校验，预先存在的非 Git 目录失败关闭。失败诊断根 `.../orca-real-ccloop-MyDo5O`、fixture `/private/tmp/orca-ccloop-d3-task8`、两开发树、node_modules 与全部 SDD 证据保留。最终真实 `/Users/biran/.orca` 仍不存在，没有真实模型调用。

下一步：Web 可恢复任务控制 → 自动拆分 → ccmem；agent 适配继续留在 ccloop。原 Codex 真钱三阶段功能成功但 wrapper 修正版未真钱重跑，不能升级表述。整合、push、分支/worktree/证据清理由人操作。Claude 额度在 2026-09-22 09:00 Asia/Shanghai 后再核实；chain 活验仍须人选 model 并明确点头。
