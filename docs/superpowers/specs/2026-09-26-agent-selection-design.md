...
### 13.6 终审 I-2 的人裁与落地（2026-09-26，控制器会话 `8c6302e0`；13.1–13.5 原文保留）

- **人裁**（原话）：「I-2 只选 (a)。或者说 (c) 可以有但是默认允许claude 等agent升级。我们希望保留 agent 升级获取更多功能的能力。」⇒ 采 (a)；**不**默认关闭 agent 自动更新；(b) 不做。
- **更正 §4 表格「`configHash` ＝ 物化配置 `{schema, kind, installation, selection}` 的 canonical hash」**：现为**去掉 `installation.version` 之后**的 canonical hash（ccloop `src/agents/materialize.ts` 的 `agentConfigHash`）。安装记录的其余字段（`command`、`configDir`、`timeoutMs`、`killGraceMs`、kind 自带字段）与选择仍全部进哈希。§13.5 那条「登记（要人裁）」由本条关闭。
- **不变的**：`--version` 漂移检查照旧（观测到且不等 ⇒ `agent-version-drift`）；accept 封存的 `config.json` 仍含 `version`，是「这一次跑的是哪个版本」的记录。Orca 不重算 `configHash`，Orca 侧零代码改动。
- **升级后的出路**：CLI 原地升级 ⇒ 漂移检查挡住新工作（闸门路径是组级阻塞 `claim-capability-unavailable`）⇒ 人让表记下新版本（`orca agents init` 或改表）⇒ 同一个 hash 重新解析得出 ⇒ recovery-retry 后已开跑的组可继续（剩余任务、续跑、解冲突）。**「recovery-retry 清掉该阻塞并重新探测」只按终审 m-2 的描述引用，本轮未单独实测。**
...
### 13.8 独立审计的追验与补判据（2026-09-27，控制器会话 `8c6302e0`；人裁「1 追验 2 处理覆盖偏弱问题 3 一个本轮之前就有的缺口 => 需要修复」）

- **追验**：审计判为 UNVERIFIED 的 9 条改写（助手／夹具级：commandLedger `%s`、planImport SIGKILL `%s`、controlReadApi `setup()`／`profile()`／`confirmGroup()`／`insertValidTaskRun()`、driverRecovery `claimedSoft()`、webContinuationAccounting `runningTasks()`、webFaults `claimed()`、ccloopPort `probeSnapshot()`、controlConfigPort `snapshot()`）与 ccloopProtocol.integration 的 SIGKILL 两行：**全部未放宽**（每条都有一个瞄准其标题所承诺行为的变异，BASE 红、HEAD 同一断言红）。本轮之前就弱、现在仍弱的点（登记不修）：SIGKILL 两行只被一个两处改动的变异打红，单处的「同字节重发布／再走一遍已 settled 的 run」都照绿；controlReadApi 的键数检查、`objective.taskId` 核对、controlConfigPort 的 `defaults` 强制为 null 三个变异在 BASE 与 HEAD 都绿。
- **覆盖偏弱（per-task `configHash`）**：夹具对所有选择答同一个常量 `configHash`，「把别的任务的 hash 交给这个任务」在六处生产可达的位置无判据：确认冻结（`webService.ts`）、派活建 run（`webDispatch.ts`）、解冲突选择文件（`driverLanding.ts`）、读模型的 work item 对快照与 run 对 work item（`controlViews.ts` 两处，后者即 `sameFrozenRecords`）、驱动环 A2 的 work item 对快照（`executionSnapshot.ts`）。补法：夹具加可选开关 `distinctConfigHash`（缺省逐字节不变）＋ 新文件 `tests/control/configHashPerTask.test.ts` 6 条 ＋ `tests/panel/controlReadApi.test.ts` 1 条；每条在它守的变异下见红（断言红，非超时）。
