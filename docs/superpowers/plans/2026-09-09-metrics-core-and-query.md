...
*** **本文由 `docs/superpowers/plans/2026-09-09-metrics-core-and-query-v2.md` 取代。** ***
v2 是**自包含**的（执行者不需要读本文），并在开头列了一张
「v2 相对 v1 改了什么」的表，**逐条指向它修的那个发现**。

⚠️ **本文不删** —— 它是「当时相信什么」的记录，且 §5 那两条记法只有对着本文的原文才读得懂。
**要执行的是 v2，不是本文。**
